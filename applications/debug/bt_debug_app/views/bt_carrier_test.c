...
static void bt_carrier_test_start(BtCarrierTest* bt_carrier_test) {
    furry_assert(bt_carrier_test);
    if(bt_carrier_test->mode == BtTestModeRx) {
        furry_hal_bt_start_packet_rx(bt_carrier_test->channel, 1);
        furry_timer_start(bt_carrier_test->timer, furry_kernel_get_tick_frequency() / 4);
    } else if(bt_carrier_test->mode == BtTestModeTxHopping) {
        furry_hal_bt_start_tone_tx(bt_carrier_test->channel, bt_carrier_test->power);
        furry_timer_start(bt_carrier_test->timer, furry_kernel_get_tick_frequency() * 2);
...
static void bt_carrier_test_stop(BtCarrierTest* bt_carrier_test) {
    ...
    if(bt_carrier_test->mode == BtTestModeTxHopping) {
        furry_hal_bt_stop_tone_tx();
        furry_timer_stop(bt_carrier_test->timer);
    } else if(bt_carrier_test->mode == BtTestModeTx) {
        furry_hal_bt_stop_tone_tx();
...
static void bt_test_carrier_timer_callback(void* context) {
    ...
    BtCarrierTest* bt_carrier_test = context;
    if(bt_carrier_test->mode == BtTestModeRx) {
        bt_test_set_rssi(bt_carrier_test->bt_test, furry_hal_bt_get_rssi());
    } else if(bt_carrier_test->mode == BtTestModeTxHopping) {
...
BtCarrierTest* bt_carrier_test_alloc() {
    ...
    bt_carrier_test->timer =
        furry_timer_alloc(bt_test_carrier_timer_callback, FurryTimerTypePeriodic, bt_carrier_test);
...
void bt_carrier_test_free(BtCarrierTest* bt_carrier_test) {
    ...
    furry_timer_free(bt_carrier_test->timer);
