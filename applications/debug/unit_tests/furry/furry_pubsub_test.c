...
void test_furry_pubsub() {
    FurryPubSub* test_pubsub = NULL;
    FurryPubSubSubscription* test_pubsub_subscription = NULL;

    // init pubsub case
    test_pubsub = furry_pubsub_alloc();
    mu_assert_pointers_not_eq(test_pubsub, NULL);

    // subscribe pubsub case
    test_pubsub_subscription =
        furry_pubsub_subscribe(test_pubsub, test_pubsub_handler, (void*)&context_value);
    mu_assert_pointers_not_eq(test_pubsub_subscription, NULL);

    /// notify pubsub case
    furry_pubsub_publish(test_pubsub, (void*)&notify_value_0);
    mu_assert_int_eq(pubsub_value, notify_value_0);
    mu_assert_int_eq(pubsub_context_value, context_value);

    // unsubscribe pubsub case
    furry_pubsub_unsubscribe(test_pubsub, test_pubsub_subscription);

    /// notify unsubscribed pubsub case
    furry_pubsub_publish(test_pubsub, (void*)&notify_value_1);
    mu_assert_int_not_eq(pubsub_value, notify_value_1);

    // delete pubsub case
    furry_pubsub_free(test_pubsub);
}
