...
static void power_test_deinit(void) {
    ...
    furry_hal_power_set_battery_charge_voltage_limit(4.208f);
}
...
MU_TEST(test_power_charge_voltage_limit_exact) {
    ...
    for(uint16_t charge_mv = 3840; charge_mv <= 4208; charge_mv += 16) {
        ...
        furry_hal_power_set_battery_charge_voltage_limit(charge_volt);
        mu_assert_double_eq(charge_volt, furry_hal_power_get_battery_charge_voltage_limit());
    }
}
...
MU_TEST(test_power_charge_voltage_limit_floating_imprecision) {
    ...
    furry_hal_power_set_battery_charge_voltage_limit(4.016f);
    mu_assert_double_eq(4.016f, furry_hal_power_get_battery_charge_voltage_limit());
}

MU_TEST(test_power_charge_voltage_limit_inexact) {
    // Charge voltage limits that are not power of 16mV get truncated down
    furry_hal_power_set_battery_charge_voltage_limit(3.841f);
    mu_assert_double_eq(3.840, furry_hal_power_get_battery_charge_voltage_limit());

    furry_hal_power_set_battery_charge_voltage_limit(3.900f);
    mu_assert_double_eq(3.888, furry_hal_power_get_battery_charge_voltage_limit());

    furry_hal_power_set_battery_charge_voltage_limit(4.200f);
    mu_assert_double_eq(4.192, furry_hal_power_get_battery_charge_voltage_limit());
}
...
MU_TEST(test_power_charge_voltage_limit_invalid_clamped) {
    ...
    furry_hal_power_set_battery_charge_voltage_limit(3.808f);
    mu_assert_double_eq(3.840, furry_hal_power_get_battery_charge_voltage_limit());
    furry_hal_power_set_battery_charge_voltage_limit(1.0f);
    mu_assert_double_eq(3.840, furry_hal_power_get_battery_charge_voltage_limit());

    // NOTE: Intentionally picking a small increment above 4.208 V to reduce the risk of an
    // unhappy battery if this fails.
    furry_hal_power_set_battery_charge_voltage_limit(4.240f);
    mu_assert_double_eq(4.208, furry_hal_power_get_battery_charge_voltage_limit());
    ...
    furry_hal_power_set_battery_charge_voltage_limit(8.105f);
    mu_assert_double_eq(4.208, furry_hal_power_get_battery_charge_voltage_limit());
}
