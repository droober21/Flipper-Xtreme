...
DICT_DEF2(StorageTestPathDict, FurryString*, FURRY_STRING_OPLIST, StorageTestPath, M_POD_OPLIST)
...
static StorageTestPathDict_t*
    ...
    for(size_t i = 0; i < paths_count; i++) {
        FurryString* key;
        key = furry_string_alloc_set(paths[i].path);
...
static bool storage_test_paths_mark(StorageTestPathDict_t* data, FurryString* path, bool is_dir) {
    bool found = false;
...
static void storage_dirs_create(Storage* storage, const char* base) {
    ...
    for(size_t i = 0; i < COUNT_OF(storage_test_dirwalk_paths); i++) {
        furry_string_printf(path, "%s/%s", base, storage_test_dirwalk_paths[i]);
        storage_common_mkdir(storage, furry_string_get_cstr(path));
    }

    for(size_t i = 0; i < COUNT_OF(storage_test_dirwalk_files); i++) {
        furry_string_printf(path, "%s/%s", base, storage_test_dirwalk_files[i]);
...
MU_TEST_1(test_dirwalk_full, Storage* storage) {
    ...
    while(dir_walk_read(dir_walk, path, &fileinfo) == DirWalkOK) {
        furry_string_right(path, strlen(EXT_PATH("dirwalk/")));
