...
static int32_t storage_file_locker(void* ctx) {
    Storage* storage = furry_record_open(RECORD_STORAGE);
    FurrySemaphore* semaphore = ctx;
    File* file = storage_file_alloc(storage);
    furry_check(storage_file_open(file, STORAGE_LOCKED_FILE, FSAM_READ_WRITE, FSOM_OPEN_EXISTING));
    furry_semaphore_release(semaphore);
    furry_delay_ms(1000);

    furry_check(storage_file_close(file));
    furry_record_close(RECORD_STORAGE);
    storage_file_free(file);
    return 0;
}
...
MU_TEST(storage_file_open_lock) {
    ...
    FurryThread* locker_thread =
        furry_thread_alloc_ex("StorageFileLocker", 2048, storage_file_locker, semaphore);
    furry_thread_start(locker_thread);

    // wait for file lock
    furry_semaphore_acquire(semaphore, FurryWaitForever);
    ...
    mu_check(furry_thread_join(locker_thread));
    furry_thread_free(locker_thread);
...
static int32_t storage_dir_locker(void* ctx) {
    ...
    FurrySemaphore* semaphore = ctx;
    File* file = storage_file_alloc(storage);
    furry_check(storage_dir_open(file, STORAGE_LOCKED_DIR));
    furry_semaphore_release(semaphore);
    furry_delay_ms(100);

    furry_check(storage_dir_close(file));
    furry_record_close(RECORD_STORAGE);
    storage_file_free(file);
    return 0;
}
...
MU_TEST(storage_dir_open_lock) {
    ...
    // file_locker thread start
    FurryThread* locker_thread =
        furry_thread_alloc_ex("StorageDirLocker", 2048, storage_dir_locker, semaphore);
    furry_thread_start(locker_thread);
...
static void storage_dir_create(Storage* storage, const char* base) {
    ...
    for(size_t i = 0; i < COUNT_OF(storage_copy_test_paths); i++) {
        furry_string_printf(path, "%s/%s", base, storage_copy_test_paths[i]);
        storage_common_mkdir(storage, furry_string_get_cstr(path));
    }

    for(size_t i = 0; i < COUNT_OF(storage_copy_test_files); i++) {
        furry_string_printf(path, "%s/%s", base, storage_copy_test_files[i]);
        write_file_13DA(storage, furry_string_get_cstr(path));
    }
...
static bool storage_dir_rename_check(Storage* storage, const char* base) {
    ...
    if(result) {
        for(size_t i = 0; i < COUNT_OF(storage_copy_test_paths); i++) {
            furry_string_printf(path, "%s/%s", base, storage_copy_test_paths[i]);
            result = (storage_common_stat(storage, furry_string_get_cstr(path), NULL) == FSE_OK);
            if(!result) {
                break;
            }
        }
    ...
    if(result) {
        for(size_t i = 0; i < COUNT_OF(storage_copy_test_files); i++) {
            furry_string_printf(path, "%s/%s", base, storage_copy_test_files[i]);
            result = check_file_13DA(storage, furry_string_get_cstr(path));
            if(!result) {
                break;
            }
        }
...
MU_TEST(test_storage_data_path_apps) {
    for(size_t i = 0; i < storage_test_apps_count; i++) {
        FurryThread* thread =
            furry_thread_alloc_ex(storage_test_apps[i], 1024, storage_test_app, NULL);
        furry_thread_set_appid(thread, storage_test_apps[i]);
        furry_thread_start(thread);
        furry_thread_join(thread);

        mu_assert_int_eq(true, furry_thread_get_return_code(thread));

        // Check if app data dir and file exists
        Storage* storage = furry_record_open(RECORD_STORAGE);
        FurryString* expected = furry_string_alloc();
        furry_string_printf(expected, APPSDATA_APP_PATH("%s"), storage_test_apps[i]);

        mu_check(storage_dir_exists(storage, furry_string_get_cstr(expected)));
        furry_string_cat(expected, "/test");
        mu_check(storage_file_exists(storage, furry_string_get_cstr(expected)));

        furry_string_printf(expected, APPSDATA_APP_PATH("%s"), storage_test_apps[i]);
        storage_simply_remove_recursive(storage, furry_string_get_cstr(expected));

        furry_record_close(RECORD_STORAGE);

        furry_string_free(expected);
