...
int32_t example_plugins_app(void* p) {
    ...
    do {
        ...
        if(preload_res != FlipperApplicationPreloadStatusSuccess) {
            FURRY_LOG_E(TAG, "Failed to preload plugin");
            break;
        }

        if(!flipper_application_is_plugin(app)) {
            FURRY_LOG_E(TAG, "Plugin file is not a library");
            break;
        }

        FlipperApplicationLoadStatus load_status = flipper_application_map_to_memory(app);
        if(load_status != FlipperApplicationLoadStatusSuccess) {
            FURRY_LOG_E(TAG, "Failed to load plugin file");
            break;
        }
        ...
        furry_check(app_descriptor->ep_api_version == PLUGIN_API_VERSION);
        furry_check(strcmp(app_descriptor->appid, PLUGIN_APP_ID) == 0);
        ...
        FURRY_LOG_I(TAG, "Plugin method2(1337,228): %d", plugin->method2(1337, 228));
    } while(false);
