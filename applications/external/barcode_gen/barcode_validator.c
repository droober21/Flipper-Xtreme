...
int calculate_ean_upc_check_digit(BarcodeData* barcode_data) {
    ...
    for(int i = 0; i < length; i += 2) {
        odd += furry_string_get_char(barcode_data->raw_data, i) - '0';
    }

    //Get sum of even digits
    for(int i = 1; i < length; i += 2) {
        even += furry_string_get_char(barcode_data->raw_data, i) - '0';
    }
...
void ean_upc_loader(BarcodeData* barcode_data) {
    int barcode_length = furry_string_size(barcode_data->raw_data);
    ...
    for(int i = 0; i < barcode_length; i++) {
        char character = furry_string_get_char(barcode_data->raw_data, i);
    ...
    if(barcode_length == max_digit) {
        ...
        furry_string_cat(barcode_data->correct_data, barcode_data->raw_data);

        //append the check digit to the correct data string
        furry_string_set_char(barcode_data->correct_data, min_digits, check_digit_char);
    }
    ...
    if(barcode_length == min_digits) {
        ...
        //append the check digit to the correct data string
        furry_string_push_back(barcode_data->correct_data, check_digit_char);
    }
}
...
void code_39_loader(BarcodeData* barcode_data) {
    ...
    FurryString* barcode_bits = furry_string_alloc();
    FurryString* temp_string = furry_string_alloc();

    //add starting and ending *
    if(!furry_string_start_with(barcode_data->raw_data, "*")) {
        furry_string_push_back(temp_string, '*');
        furry_string_cat(temp_string, barcode_data->raw_data);
        furry_string_set(barcode_data->raw_data, temp_string);
    }

    if(!furry_string_end_with(barcode_data->raw_data, "*")) {
        furry_string_push_back(barcode_data->raw_data, '*');
    }

    furry_string_free(temp_string);
    barcode_length = furry_string_size(barcode_data->raw_data);
    ...
    if(!flipper_format_file_open_existing(ff, CODE39_DICT_FILE_PATH)) {
        FURRY_LOG_E(TAG, "Could not open file %s", CODE39_DICT_FILE_PATH);
        barcode_data->reason = MissingEncodingTable;
        barcode_data->valid = false;
    } else {
        FurryString* char_bits = furry_string_alloc();
        for(int i = 0; i < barcode_length; i++) {
            char barcode_char = toupper(furry_string_get_char(barcode_data->raw_data, i));
            ...
            if(!flipper_format_read_string(ff, current_character, char_bits)) {
                FURRY_LOG_E(TAG, "Could not read \"%c\" string", barcode_char);
                barcode_data->reason = InvalidCharacters;
                barcode_data->valid = false;
                break;
            } else {
                FURRY_LOG_I(
                    TAG, "\"%c\" string: %s", barcode_char, furry_string_get_cstr(char_bits));
                furry_string_cat(barcode_bits, char_bits);
            }
            flipper_format_rewind(ff);
        }
        furry_string_free(char_bits);
    }
    ...
    furry_string_cat(barcode_data->correct_data, barcode_bits);
    furry_string_free(barcode_bits);
}
...
void code_128_loader(BarcodeData* barcode_data) {
    ...
    */
    ...
    //add the start code
    furry_string_cat(barcode_bits, start_code_bits);

    if(!flipper_format_file_open_existing(ff, CODE128_DICT_FILE_PATH)) {
        FURRY_LOG_E(TAG, "Could not open file %s", CODE128_DICT_FILE_PATH);
        barcode_data->reason = MissingEncodingTable;
        barcode_data->valid = false;
    } else {
        ...
        FurryString* char_bits = furry_string_alloc();
        for(int i = 0; i < barcode_length; i++) {
            char barcode_char = furry_string_get_char(barcode_data->raw_data, i);
            ...
            if(!flipper_format_read_string(ff, furry_string_get_cstr(value), char_bits)) {
                FURRY_LOG_E(TAG, "Could not read \"%c\" string", barcode_char);
                ...
                checksum_digits += 1;
                checksum_adder += (atoi(furry_string_get_cstr(value)) * checksum_digits);

                FURRY_LOG_D(
                    TAG,
                    "\"%c\" string: %s : %s : %d : %d : %d",
                    barcode_char,
                    furry_string_get_cstr(char_bits),
                    furry_string_get_cstr(value),
                    checksum_digits,
                    (atoi(furry_string_get_cstr(value)) * checksum_digits),
                    checksum_adder);
            }
        ...
        if(!flipper_format_read_string(ff, final_check_digit_string, char_bits)) {
            FURRY_LOG_E(TAG, "Could not read \"%s\" string", final_check_digit_string);
            barcode_data->reason = EncodingTableError;
            barcode_data->valid = false;
        } else {
    ...
    //add the stop code
    furry_string_cat(barcode_bits, stop_code_bits);
