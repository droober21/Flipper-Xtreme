...
static void text_input_callback(void* ctx) {
    ...
    furry_mutex_acquire(caesar_state->mutex, FurryWaitForever);

    FURRY_LOG_D("caesar_cipher", "Input text: %s", caesar_state->input);
    // this is where we build the output.
    string_to_uppercase(caesar_state->input);
    FURRY_LOG_D("caesar_cipher", "Upper text: %s", caesar_state->input);
    ...
    furry_mutex_release(caesar_state->mutex);
}
...
int32_t caesar_cipher_app() {
    ...
    FURRY_LOG_D("caesar_cipher", "Running caesar_cipher_state_init");
    caesar_cipher_state_init(caesar_state);

    caesar_state->mutex = furry_mutex_alloc(FurryMutexTypeNormal);
    if(!caesar_state->mutex) {
        FURRY_LOG_E("caesar_cipher", "cannot create mutex\r\n");
        free(caesar_state);
        return 255;
    }

    FURRY_LOG_D("caesar_cipher", "Assigning text input callback");
    ...
    FURRY_LOG_D("caesar_cipher", "Enabling view dispatcher queue");
    view_dispatcher_enable_queue(caesar_state->view_dispatcher);

    FURRY_LOG_D("caesar_cipher", "Adding text input view to dispatcher");
    ...
    FURRY_LOG_D("caesar_cipher", "Attaching view dispatcher to GUI");
    view_dispatcher_attach_to_gui(
        caesar_state->view_dispatcher, gui, ViewDispatcherTypeFullscreen);
    FURRY_LOG_D("ceasar_cipher", "starting view dispatcher");
    ...
    furry_mutex_free(caesar_state->mutex);
