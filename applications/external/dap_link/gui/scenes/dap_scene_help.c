...
void dap_scene_help_on_enter(void* context) {
    ...
    furry_string_cat(string, "CMSIS DAP/DAP Link v2\r\n");
    furry_string_cat_printf(string, "Serial: %s\r\n", dap_app_get_serial(app->dap_app));
    furry_string_cat(
        string,
        "Pinout:\r\n"
        "\e#SWD:\r\n");
    ...
    furry_string_cat(string, "\e#JTAG:\r\n");
    switch(config->swd_pins) {
    case DapSwdPinsPA7PA6:
        furry_string_cat(
            string,
    ...
    furry_string_cat(string, "\e#UART:\r\n");
    ...
    widget_add_text_scroll_element(app->widget, 0, 0, 128, 64, furry_string_get_cstr(string));
