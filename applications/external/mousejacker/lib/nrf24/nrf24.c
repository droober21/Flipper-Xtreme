...
uint8_t
    nrf24_write_buf_reg(FurryHalSpiBusHandle* handle, uint8_t reg, uint8_t* data, uint8_t size) {
    uint8_t tx[size + 1];
    uint8_t rx[size + 1];
    memset(rx, 0, size + 1);
...
uint8_t nrf24_get_packetlen(FurryHalSpiBusHandle* handle) {
    uint8_t len = 0;
...
uint8_t
    nrf24_rxpacket(FurryHalSpiBusHandle* handle, uint8_t* packet, uint8_t* packetsize, bool full) {
    uint8_t status = 0;
    uint8_t size = 0;
