...
static void music_player_cli(Cli* cli, FurryString* args, void* context) {
    UNUSED(context);
    ...
    do {
        if(storage_common_stat(storage, furry_string_get_cstr(args), NULL) == FSE_OK) {
            if(!music_player_worker_load(music_player_worker, furry_string_get_cstr(args))) {
                printf("Failed to open file %s\r\n", furry_string_get_cstr(args));
                break;
            }
        } else {
            if(!music_player_worker_load_rtttl_from_string(
                   music_player_worker, furry_string_get_cstr(args))) {
