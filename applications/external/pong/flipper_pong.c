...
static void draw_callback(Canvas* canvas, void* ctx) {
    ...
    furry_mutex_acquire(playersMutex->mutex, FurryWaitForever);
    ...
    furry_mutex_release(playersMutex->mutex);
}
...
uint8_t changeSpeed() {
    ...
    while(1) {
        furry_hal_random_fill_buf(randomuint8, 1);
...
int32_t flipper_pong_app(void* p) {
    ...
    players.mutex = furry_mutex_alloc(FurryMutexTypeNormal);
    if(!players.mutex) {
    ...
    furry_message_queue_free(event_queue);
    furry_mutex_free(players.mutex);
