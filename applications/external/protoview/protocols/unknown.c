...
static bool decode(uint8_t* bits, uint32_t numbytes, uint32_t numbits, ProtoViewMsgInfo* info) {
    ...
    if(manchester_bits > minbits && manchester_bits > pwm3_bits && manchester_bits > pwm4_bits) {
        ...
        FURRY_LOG_E(TAG, "MANCHESTER START: %lu", tmp1);
    }
    ...
    if(preamble_found) FURRY_LOG_E(TAG, "PREAMBLE AT: %lu", preamble_start);
    FURRY_LOG_E(TAG, "START: %lu", info->start_off);
    FURRY_LOG_E(TAG, "MSGBITS: %lu", msgbits);
    FURRY_LOG_E(TAG, "DATASTART: %lu", start1);
    FURRY_LOG_E(TAG, "PULSES: %lu", info->pulses_count);
