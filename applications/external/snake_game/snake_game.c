...
static void snake_game_render_callback(Canvas* const canvas, void* ctx) {
    ...
    furry_mutex_acquire(snake_state->mutex, FurryWaitForever);
    ...
    furry_mutex_release(snake_state->mutex);
}

static void snake_game_input_callback(InputEvent* input_event, FurryMessageQueue* event_queue) {
...
static void snake_game_update_timer_callback(FurryMessageQueue* event_queue) {
    furry_assert(event_queue);
...
int32_t snake_game_app(void* p) {
    ...
    FurryMessageQueue* event_queue = furry_message_queue_alloc(8, sizeof(SnakeEvent));

    SnakeState* snake_state = malloc(sizeof(SnakeState));
    snake_game_init_game(snake_state);

    snake_state->mutex = furry_mutex_alloc(FurryMutexTypeNormal);

    if(!snake_state->mutex) {
        FURRY_LOG_E("SnakeGame", "cannot create mutex\r\n");
    ...
    FurryTimer* timer =
        furry_timer_alloc(snake_game_update_timer_callback, FurryTimerTypePeriodic, event_queue);
    ...
    furry_message_queue_free(event_queue);
    furry_mutex_free(snake_state->mutex);
    free(snake_state);
