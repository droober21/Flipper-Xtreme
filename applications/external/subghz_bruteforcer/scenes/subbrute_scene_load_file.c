...
void subbrute_scene_load_file_on_enter(void* context) {
    ...
    FurryString* app_folder;
    FurryString* load_path;
    load_path = furry_string_alloc();
    app_folder = furry_string_alloc_set(SUBBRUTE_PATH);
    ...
    bool res = true;
    furry_string_printf(load_path, "%s", "/ext/subghz/princeton.sub");
#else
    bool res =
    ...
    FURRY_LOG_D(
        ...
        furry_string_get_cstr(load_path),
        furry_string_get_cstr(app_folder));
#endif
    if(res) {
        load_result =
            subbrute_device_load_from_file(instance->device, furry_string_get_cstr(load_path));
        ...
        if(load_result == SubBruteFileResultOk) {
            ...
            FURRY_LOG_E(TAG, "Returned error: %d", load_result);

            FurryString* dialog_msg;
            dialog_msg = furry_string_alloc();
            furry_string_cat_printf(
                dialog_msg, "Cannot parse\nfile: %s", subbrute_device_error_get_desc(load_result));
            dialog_message_show_storage_error(instance->dialogs, furry_string_get_cstr(dialog_msg));
            furry_string_free(dialog_msg);
    ...
    furry_string_free(app_folder);
    furry_string_free(load_path);
}
