...
bool usb_uart_tx_data(UsbUart* usb_uart, uint8_t* data, size_t length) {
    ...
    while(pos < length) {
        ...
        if(furry_mutex_acquire(usb_uart->usb_mutex, 100) != FurryStatusOk) {
            furry_semaphore_release(usb_uart->tx_sem);
            return false;
        }
        furry_hal_cdc_send(usb_uart->cfg.vcp_ch, &data[pos], pkt_size);
        furry_mutex_release(usb_uart->usb_mutex);
...
static int32_t usb_uart_worker(void* context) {
    ...
    while(1) {
        uint32_t events =
            furry_thread_flags_wait(WORKER_ALL_EVENTS, FurryFlagWaitAny, FurryWaitForever);
        ...
        if(events & WorkerEvtCdcRx) {
            size_t len = 0;
            if(furry_mutex_acquire(usb_uart->usb_mutex, 100) == FurryStatusOk) {
                len = furry_hal_cdc_receive(usb_uart->cfg.vcp_ch, &data[remain], USB_CDC_PKT_LEN);
                furry_mutex_release(usb_uart->usb_mutex);
            }
...
static void vcp_on_cdc_rx(void* context) {
    UsbUart* usb_uart = (UsbUart*)context;
    furry_thread_flags_set(furry_thread_get_id(usb_uart->thread), WorkerEvtCdcRx);
}
