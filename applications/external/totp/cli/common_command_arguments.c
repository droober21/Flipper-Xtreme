...
bool totp_cli_try_read_digits(
    ...
    FurryString* args,
    bool* parsed) {
    if(furry_string_cmpi_str(arg, TOTP_CLI_COMMAND_ARG_DIGITS_PREFIX) == 0) {
        uint8_t digit_value;
...
bool totp_cli_try_read_duration(
    ...
    FurryString* args,
    bool* parsed) {
    if(furry_string_cmpi_str(arg, TOTP_CLI_COMMAND_ARG_DURATION_PREFIX) == 0) {
...
bool totp_cli_try_read_automation_features(
    ...
    FurryString* args,
    bool* parsed) {
    if(furry_string_cmpi_str(arg, TOTP_CLI_COMMAND_ARG_AUTOMATION_FEATURE_PREFIX) == 0) {
        if(!args_read_string_and_trim(args, arg)) {
...
bool totp_cli_try_read_unsecure_flag(const FurryString* arg, bool* parsed, bool* unsecure_flag) {
    if(furry_string_cmpi_str(arg, TOTP_CLI_COMMAND_ARG_UNSECURE_PREFIX) == 0) {
        *unsecure_flag = false;
        *parsed = true;
        return true;
    }
...
bool totp_cli_try_read_plain_token_secret_encoding(
    ...
    if(furry_string_cmpi_str(arg, TOTP_CLI_COMMAND_ARG_SECRET_ENCODING_PREFIX) == 0) {
        if(!args_read_string_and_trim(args, arg)) {
            totp_cli_printf_missed_argument_value(TOTP_CLI_COMMAND_ARG_SECRET_ENCODING_PREFIX);
        } else {
            if(furry_string_cmpi_str(arg, PLAIN_TOKEN_ENCODING_BASE32_NAME) == 0) {
                *secret_encoding = PlainTokenSecretEncodingBase32;
                *parsed = true;
            } else if(furry_string_cmpi_str(arg, PLAIN_TOKEN_ENCODING_BASE64_NAME) == 0) {
