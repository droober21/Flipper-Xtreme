...
static Storage* totp_open_storage() {
    return furry_record_open(RECORD_STORAGE);
}
...
static char* totp_config_file_backup_i(Storage* storage) {
    ...
    FurryHalRtcDateTime current_datetime;
    furry_hal_rtc_get_datetime(&current_datetime);

    uint8_t backup_path_size = sizeof(CONFIG_FILE_BACKUP_BASE_PATH) + 14;
    char* backup_path = malloc(backup_path_size);
    furry_check(backup_path != NULL);
    ...
    if(backup_file_exists ||
       storage_common_copy(storage, CONFIG_FILE_PATH, backup_path) != FSE_OK) {
        FURRY_LOG_E(LOGGING_TAG, "Unable to take a backup");
        free(backup_path);
        return NULL;
    }

    FURRY_LOG_I(LOGGING_TAG, "Took config file backup to %s", backup_path);
    return backup_path;
}
...
static bool totp_open_config_file(Storage* storage, FlipperFormat** file) {
    ...
    if(storage_common_stat(storage, CONFIG_FILE_PATH, NULL) == FSE_OK) {
        FURRY_LOG_D(LOGGING_TAG, "Config file %s found", CONFIG_FILE_PATH);
        if(!flipper_format_file_open_existing(fff_data_file, CONFIG_FILE_PATH)) {
            FURRY_LOG_E(LOGGING_TAG, "Error opening existing file %s", CONFIG_FILE_PATH);
            totp_close_config_file(fff_data_file);
            return false;
        }
    } else if(storage_common_stat(storage, CONFIG_FILE_PATH_PREVIOUS, NULL) == FSE_OK) {
        FURRY_LOG_D(LOGGING_TAG, "Old config file %s found", CONFIG_FILE_PATH_PREVIOUS);
        if(storage_common_stat(storage, CONFIG_FILE_DIRECTORY_PATH, NULL) == FSE_NOT_EXIST) {
            ...
            if(!storage_simply_mkdir(storage, CONFIG_FILE_DIRECTORY_PATH)) {
                FURRY_LOG_E(LOGGING_TAG, "Error creating directory %s", CONFIG_FILE_DIRECTORY_PATH);
                totp_close_config_file(fff_data_file);
                return false;
            }
        }
        if(storage_common_rename(storage, CONFIG_FILE_PATH_PREVIOUS, CONFIG_FILE_PATH) != FSE_OK) {
            FURRY_LOG_E(LOGGING_TAG, "Error moving config to %s", CONFIG_FILE_PATH);
            totp_close_config_file(fff_data_file);
            return false;
        }
        FURRY_LOG_I(LOGGING_TAG, "Applied config file path migration");
        return totp_open_config_file(storage, file);
    } else {
        FURRY_LOG_D(LOGGING_TAG, "Config file %s is not found. Will create new.", CONFIG_FILE_PATH);
        if(storage_common_stat(storage, CONFIG_FILE_DIRECTORY_PATH, NULL) == FSE_NOT_EXIST) {
            FURRY_LOG_D(
                LOGGING_TAG,
                "Directory %s doesn't exist. Will create new.",
        ...
        if(!flipper_format_file_open_new(fff_data_file, CONFIG_FILE_PATH)) {
            totp_close_config_file(fff_data_file);
            FURRY_LOG_E(LOGGING_TAG, "Error creating new file %s", CONFIG_FILE_PATH);
            return false;
        }
        ...
        if(!flipper_format_rewind(fff_data_file)) {
            totp_close_config_file(fff_data_file);
            FURRY_LOG_E(LOGGING_TAG, "Rewind error");
            return false;
        }
    }
...
bool totp_config_file_load(PluginState* const plugin_state) {
    ...
    do {
        ...
        if(!flipper_format_read_header(fff_data_file, temp_str, &file_version)) {
            FURRY_LOG_E(LOGGING_TAG, "Missing or incorrect header");
            break;
        }
        ...
        if(!flipper_format_read_hex(
            ...
            FURRY_LOG_D(LOGGING_TAG, "Missing base IV");
        }
        ...
        if(flipper_format_get_value_count(
           ...
           crypto_size > 0) {
            ...
            if(!flipper_format_read_hex(
                   ...
                   crypto_size)) {
                FURRY_LOG_D(LOGGING_TAG, "Missing crypto verify token");
        ...
        if(!flipper_format_read_float(
            ...
            plugin_state->timezone_offset = 0;
            FURRY_LOG_D(LOGGING_TAG, "Missing timezone offset information, defaulting to 0");
        }
        ...
        furry_check(plugin_state->config_file_context != NULL);
    ...
    while(true) {
        ...
        if(strncmp(buffer, "\n" TOTP_CONFIG_KEY_TOKEN_SECRET ":", sizeof(buffer)) == 0) {
            ...
            if(secret_bytes_count > 1) {
                ...
                furry_check(encrypted_token != NULL);
