...
static int32_t totp_generate_worker_callback(void* context) {
    ...
    while(true) {
        ...
        if(flags ==
           (uint32_t)
               FurryFlagErrorTimeout) { // If timeout, consider as no error, as we expect this and can handle gracefully
            flags = 0;
        }
        ...
        uint32_t curr_ts = furry_hal_rtc_get_timestamp();

        bool time_left = false;
        if(flags & TotpGenerateCodeWorkerEventForceUpdate ||
           (time_left = (curr_ts % token_info->duration) == 0)) {
            if(furry_mutex_acquire(t_context->code_buffer_sync, FurryWaitForever) == FurryStatusOk) {
                generate_totp_code(t_context, token_info, curr_ts);
                curr_ts = furry_hal_rtc_get_timestamp();
                furry_mutex_release(t_context->code_buffer_sync);
...
TotpGenerateCodeWorkerContext* totp_generate_code_worker_start(
    ...
    furry_thread_set_name(context->thread, "TOTPGenerateWorker");
    furry_thread_set_stack_size(context->thread, 2048);
    furry_thread_set_context(context->thread, context);
    furry_thread_set_callback(context->thread, totp_generate_worker_callback);
...
void totp_generate_code_worker_stop(TotpGenerateCodeWorkerContext* context) {
    furry_check(context != NULL);
    furry_thread_flags_set(furry_thread_get_id(context->thread), TotpGenerateCodeWorkerEventStop);
