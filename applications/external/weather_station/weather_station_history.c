...
bool ws_history_get_text_space_left(WSHistory* instance, FurryString* output) {
    furry_assert(instance);
    ...
    if(output != NULL)
        furry_string_printf(output, "%02u/%02u", instance->last_index_write, WS_HISTORY_MAX);
    return false;
}

void ws_history_get_text_item_menu(WSHistory* instance, FurryString* output, uint16_t idx) {
    WSHistoryItem* item = WSHistoryItemArray_get(instance->history->data, idx);
    ...
    if(!sensor_found) { //-V547
        ...
        do {
            ...
            if(temp_data != WS_NO_CHANNEL) {
                furry_string_cat_printf(instance->tmp_string, " Ch:%X", (uint8_t)temp_data);
            }

            furry_string_printf(
                item->item_str, "%s %llX", furry_string_get_cstr(instance->tmp_string), data);
