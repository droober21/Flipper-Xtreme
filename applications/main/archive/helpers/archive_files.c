...
void archive_set_file_type(ArchiveFile_t* file, const char* path, bool is_folder, bool is_app) {
    ...
    if(is_app) {
        ...
        for(size_t i = 0; i < COUNT_OF(known_ext); i++) {
            ...
            if(furry_string_search(file->path, known_ext[i], 0) != FURRY_STRING_FAILURE) {
                if(i == ArchiveFileTypeBadKb) {
                    if(furry_string_search(file->path, archive_get_default_path(ArchiveTabBadKb)) ==
                       0) {
                        file->type = i;
...
void archive_file_append(const char* path, const char* format, ...) {
    ...
    if(res) {
        storage_file_write(file, furry_string_get_cstr(string), furry_string_size(string));
    }
...
void archive_delete_file(void* context, const char* format, ...) {
    ...
    FileInfo fileinfo;
    storage_common_stat(fs_api, furry_string_get_cstr(filename), &fileinfo);

    bool res = false;

    if(file_info_is_dir(&fileinfo)) {
        res = storage_simply_remove_recursive(fs_api, furry_string_get_cstr(filename));
    } else {
        res = (storage_common_remove(fs_api, furry_string_get_cstr(filename)) == FSE_OK);
    }

    furry_record_close(RECORD_STORAGE);

    if(archive_is_favorite("%s", furry_string_get_cstr(filename))) {
        archive_favorites_delete("%s", furry_string_get_cstr(filename));
    }
...
FS_Error archive_rename_file_or_dir(void* context, const char* src_path, const char* dst_path) {
    ...
    FURRY_LOG_I(TAG, "Rename from %s to %s", src_path, dst_path);
    ...
    if(error == FSE_OK || error == FSE_EXIST) {
        FURRY_LOG_I(TAG, "Rename from %s to %s is DONE", src_path, dst_path);
