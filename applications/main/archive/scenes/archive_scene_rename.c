...
void archive_scene_rename_on_enter(void* context) {
    ...
    FurryString* path_name;
    path_name = furry_string_alloc();

    if(current->type == ArchiveFileTypeFolder) {
        path_extract_basename(furry_string_get_cstr(current->path), path_name);
        strlcpy(archive->text_store, furry_string_get_cstr(path_name), MAX_NAME_LEN);
...
bool archive_scene_rename_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == SCENE_RENAME_CUSTOM_EVENT) {
            ...
            FurryString* path_dst;

            path_dst = furry_string_alloc();

            if(file->type == ArchiveFileTypeFolder) {
                // Rename folder/dir
                path_extract_dirname(path_src, path_dst);
                furry_string_cat_printf(path_dst, "/%s", archive->text_store);
            ...
            if(error == FSE_OK || error == FSE_EXIST) {
                ...
                furry_string_cat_printf(dialog_msg, "Cannot rename\nCode: %d", error);
                dialog_message_show_storage_error(
                    archive->dialogs, furry_string_get_cstr(dialog_msg));
