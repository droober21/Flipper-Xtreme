...
static bool text_show_read_lines(File* file, FurryString* str_result) {
...
void archive_scene_show_on_enter(void* context) {
    ...
    FileInfo fileinfo;
    FS_Error error = storage_common_stat(fs_api, furry_string_get_cstr(current->path), &fileinfo);
    if(error == FSE_OK) {
        if((fileinfo.size < SHOW_MAX_FILE_SIZE) && (fileinfo.size > 2)) {
            bool ok = storage_file_open(
                file, furry_string_get_cstr(current->path), FSAM_READ, FSOM_OPEN_EXISTING);
            if(ok) {
                ...
                widget_add_text_scroll_element(
                    instance->widget, 0, 0, 128, 64, furry_string_get_cstr(buffer));
