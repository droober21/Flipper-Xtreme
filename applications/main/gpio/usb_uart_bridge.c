...
struct UsbUartBridge {
    ...
    FurryThread* thread;
    FurryThread* tx_thread;

    FurryStreamBuffer* rx_stream;

    FurryMutex* usb_mutex;

    FurrySemaphore* tx_sem;

    UsbUartState st;

    FurryApiLock cfg_lock;
...
static void usb_uart_on_irq_cb(UartIrqEvent ev, uint8_t data, void* context) {
    ...
    if(ev == UartIrqEventRXNE) {
        furry_stream_buffer_send(usb_uart->rx_stream, &data, 1, 0);
        furry_thread_flags_set(furry_thread_get_id(usb_uart->thread), WorkerEvtRxDone);
    }
}
...
static void usb_uart_vcp_init(UsbUartBridge* usb_uart, uint8_t vcp_ch) {
    ...
    furry_hal_cdc_set_callbacks(vcp_ch, (CdcCallbacks*)&cdc_cb, usb_uart);
}

static void usb_uart_vcp_deinit(UsbUartBridge* usb_uart, uint8_t vcp_ch) {
    UNUSED(usb_uart);
    furry_hal_cdc_set_callbacks(vcp_ch, NULL, NULL);
    if(vcp_ch != 0) {
        Cli* cli = furry_record_open(RECORD_CLI);
        cli_session_close(cli);
        furry_record_close(RECORD_CLI);
...
static void usb_uart_serial_init(UsbUartBridge* usb_uart, uint8_t uart_ch) {
    if(uart_ch == FurryHalUartIdUSART1) {
        furry_hal_console_disable();
    } else if(uart_ch == FurryHalUartIdLPUART1) {
        furry_hal_uart_init(uart_ch, 115200);
    }
    furry_hal_uart_set_irq_cb(uart_ch, usb_uart_on_irq_cb, usb_uart);
}
...
static void usb_uart_serial_deinit(UsbUartBridge* usb_uart, uint8_t uart_ch) {
    ...
    if(uart_ch == FurryHalUartIdUSART1)
        ...
        furry_hal_uart_deinit(uart_ch);
}

static void usb_uart_set_baudrate(UsbUartBridge* usb_uart, uint32_t baudrate) {
    if(baudrate != 0) {
        furry_hal_uart_set_br(usb_uart->cfg.uart_ch, baudrate);
        usb_uart->st.baudrate_cur = baudrate;
    } else {
        struct usb_cdc_line_coding* line_cfg =
            furry_hal_cdc_get_port_settings(usb_uart->cfg.vcp_ch);
        if(line_cfg->dwDTERate > 0) {
            furry_hal_uart_set_br(usb_uart->cfg.uart_ch, line_cfg->dwDTERate);
...
static void usb_uart_update_ctrl_lines(UsbUartBridge* usb_uart) {
    if(usb_uart->cfg.flow_pins != 0) {
        furry_assert((size_t)(usb_uart->cfg.flow_pins - 1) < COUNT_OF(flow_pins));
        uint8_t state = furry_hal_cdc_get_ctrl_line_state(usb_uart->cfg.vcp_ch);

        furry_hal_gpio_write(flow_pins[usb_uart->cfg.flow_pins - 1][0], !(state & USB_CDC_BIT_RTS));
        furry_hal_gpio_write(flow_pins[usb_uart->cfg.flow_pins - 1][1], !(state & USB_CDC_BIT_DTR));
    }
}
...
static int32_t usb_uart_worker(void* context) {
    ...
    usb_uart->rx_stream = furry_stream_buffer_alloc(USB_UART_RX_BUF_SIZE, 1);

    usb_uart->tx_sem = furry_semaphore_alloc(1, 1);
    usb_uart->usb_mutex = furry_mutex_alloc(FurryMutexTypeNormal);

    usb_uart->tx_thread =
        furry_thread_alloc_ex("UsbUartTxWorker", 512, usb_uart_tx_thread, usb_uart);
    ...
    furry_thread_flags_set(furry_thread_get_id(usb_uart->tx_thread), WorkerEvtCdcRx);

    furry_thread_start(usb_uart->tx_thread);

    while(1) {
        uint32_t events =
            furry_thread_flags_wait(WORKER_ALL_RX_EVENTS, FurryFlagWaitAny, FurryWaitForever);
        furry_check(!(events & FurryFlagError));
        if(events & WorkerEvtStop) break;
        if(events & WorkerEvtRxDone) {
            size_t len = furry_stream_buffer_receive(
                usb_uart->rx_stream, usb_uart->rx_buf, USB_CDC_PKT_LEN, 0);
            if(len > 0) {
                if(furry_semaphore_acquire(usb_uart->tx_sem, 100) == FurryStatusOk) {
                    usb_uart->st.rx_cnt += len;
                    furry_check(
                        furry_mutex_acquire(usb_uart->usb_mutex, FurryWaitForever) == FurryStatusOk);
                    furry_hal_cdc_send(usb_uart->cfg.vcp_ch, usb_uart->rx_buf, len);
                    furry_check(furry_mutex_release(usb_uart->usb_mutex) == FurryStatusOk);
                } else {
                    furry_stream_buffer_reset(usb_uart->rx_stream);
                }
            }
        }
        if(events & WorkerEvtCfgChange) {
            if(usb_uart->cfg.vcp_ch != usb_uart->cfg_new.vcp_ch) {
                furry_thread_flags_set(furry_thread_get_id(usb_uart->tx_thread), WorkerEvtTxStop);
                furry_thread_join(usb_uart->tx_thread);
            ...
            if(usb_uart->cfg.flow_pins != usb_uart->cfg_new.flow_pins) {
                ...
                if(usb_uart->cfg_new.flow_pins != 0) {
                    furry_assert((size_t)(usb_uart->cfg_new.flow_pins - 1) < COUNT_OF(flow_pins));
                    furry_hal_gpio_init_simple(
    ...
    if(usb_uart->cfg.flow_pins != 0) {
        furry_hal_gpio_init_simple(flow_pins[usb_uart->cfg.flow_pins - 1][0], GpioModeAnalog);
        furry_hal_gpio_init_simple(flow_pins[usb_uart->cfg.flow_pins - 1][1], GpioModeAnalog);
    }

    furry_thread_flags_set(furry_thread_get_id(usb_uart->tx_thread), WorkerEvtTxStop);
    furry_thread_join(usb_uart->tx_thread);
    furry_thread_free(usb_uart->tx_thread);

    furry_stream_buffer_free(usb_uart->rx_stream);
    furry_mutex_free(usb_uart->usb_mutex);
    furry_semaphore_free(usb_uart->tx_sem);
...
static int32_t usb_uart_tx_thread(void* context) {
    ...
    while(1) {
        uint32_t events =
            furry_thread_flags_wait(WORKER_ALL_TX_EVENTS, FurryFlagWaitAny, FurryWaitForever);
        furry_check(!(events & FurryFlagError));
        if(events & WorkerEvtTxStop) break;
        if(events & WorkerEvtCdcRx) {
            furry_check(furry_mutex_acquire(usb_uart->usb_mutex, FurryWaitForever) == FurryStatusOk);
            size_t len = furry_hal_cdc_receive(usb_uart->cfg.vcp_ch, data, USB_CDC_PKT_LEN);
            furry_check(furry_mutex_release(usb_uart->usb_mutex) == FurryStatusOk);

            if(len > 0) {
                usb_uart->st.tx_cnt += len;
                furry_hal_uart_tx(usb_uart->cfg.uart_ch, data, len);
            }
        }
    }
    return 0;
...
static void vcp_on_cdc_tx_complete(void* context) {
    UsbUartBridge* usb_uart = (UsbUartBridge*)context;
    furry_semaphore_release(usb_uart->tx_sem);
}
...
static void vcp_on_cdc_control_line(void* context, uint8_t state) {
    UNUSED(state);
    UsbUartBridge* usb_uart = (UsbUartBridge*)context;
    furry_thread_flags_set(furry_thread_get_id(usb_uart->thread), WorkerEvtCtrlLineSet);
}

static void vcp_on_line_config(void* context, struct usb_cdc_line_coding* config) {
    UNUSED(config);
    UsbUartBridge* usb_uart = (UsbUartBridge*)context;
    furry_thread_flags_set(furry_thread_get_id(usb_uart->thread), WorkerEvtLineCfgSet);
}
...
UsbUartBridge* usb_uart_enable(UsbUartConfig* cfg) {
    ...
    usb_uart->thread = furry_thread_alloc_ex("UsbUartWorker", 1024, usb_uart_worker, usb_uart);

    furry_thread_start(usb_uart->thread);
    return usb_uart;
}

void usb_uart_disable(UsbUartBridge* usb_uart) {
    furry_assert(usb_uart);
    furry_thread_flags_set(furry_thread_get_id(usb_uart->thread), WorkerEvtStop);
    furry_thread_join(usb_uart->thread);
    furry_thread_free(usb_uart->thread);
    free(usb_uart);
}
...
void usb_uart_set_config(UsbUartBridge* usb_uart, UsbUartConfig* cfg) {
    ...
    memcpy(&(usb_uart->cfg_new), cfg, sizeof(UsbUartConfig));
    furry_thread_flags_set(furry_thread_get_id(usb_uart->thread), WorkerEvtCfgChange);
