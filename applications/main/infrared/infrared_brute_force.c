...
struct InfraredBruteForce {
    ...
    FurryString* current_record_name;
...
InfraredBruteForce* infrared_brute_force_alloc() {
    ...
    brute_force->current_record_name = furry_string_alloc();
    InfraredBruteForceRecordDict_init(brute_force->records);
    return brute_force;
}

void infrared_brute_force_free(InfraredBruteForce* brute_force) {
    furry_assert(!brute_force->is_started);
    InfraredBruteForceRecordDict_clear(brute_force->records);
    furry_string_free(brute_force->current_record_name);
    free(brute_force);
}
...
bool infrared_brute_force_calculate_messages(InfraredBruteForce* brute_force) {
    furry_assert(!brute_force->is_started);
    furry_assert(brute_force->db_filename);
    bool success = false;
    ...
    if(success) {
        FurryString* signal_name;
        signal_name = furry_string_alloc();
    ...
    for(InfraredBruteForceRecordDict_it(it, brute_force->records);
        ...
        if(record->value.index == index) {
            *record_count = record->value.count;
            if(*record_count) {
                furry_string_set(brute_force->current_record_name, record->key);
            }
            break;
        }
    }
...
void infrared_brute_force_stop(InfraredBruteForce* brute_force) {
    furry_assert(brute_force->is_started);
    furry_string_reset(brute_force->current_record_name);
...
void infrared_brute_force_add_record(
    ...
    FurryString* key;
    key = furry_string_alloc_set(name);
