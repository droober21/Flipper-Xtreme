...
static void lfrfid_cli(Cli* cli, FurryString* args, void* context);
...
static void lfrfid_cli_read_callback(LFRFIDWorkerReadResult result, ProtocolId proto, void* ctx) {
    ...
    furry_event_flag_set(context->event, 1 << result);
}

static void lfrfid_cli_read(Cli* cli, FurryString* args) {
    FurryString* type_string;
    type_string = furry_string_alloc();
    LFRFIDWorkerReadType type = LFRFIDWorkerReadTypeAuto;

    if(args_read_string_and_trim(args, type_string)) {
        if(furry_string_cmp_str(type_string, "normal") == 0 ||
           furry_string_cmp_str(type_string, "ask") == 0) {
            // ask
            type = LFRFIDWorkerReadTypeASKOnly;
        } else if(
            furry_string_cmp_str(type_string, "indala") == 0 ||
            furry_string_cmp_str(type_string, "psk") == 0) {
            // psk
            type = LFRFIDWorkerReadTypePSKOnly;
        } else {
            lfrfid_cli_print_usage();
            furry_string_free(type_string);
            return;
        }
    }
    furry_string_free(type_string);
    ...
    context.event = furry_event_flag_alloc();
    ...
    while(true) {
        uint32_t flags =
            furry_event_flag_wait(context.event, available_flags, FurryFlagWaitAny, 100);

        if(flags != (unsigned)FurryFlagErrorTimeout) {
            if(FURRY_BIT(flags, LFRFIDWorkerReadDone)) {
                break;
            }
        }
    ...
    if(context.protocol != PROTOCOL_NO) {
        ...
        FurryString* info;
        info = furry_string_alloc();
        protocol_dict_render_data(dict, info, context.protocol);
        if(!furry_string_empty(info)) {
            printf("%s\r\n", furry_string_get_cstr(info));
        }
        furry_string_free(info);
    }

    printf("Reading stopped\r\n");
    protocol_dict_free(dict);

    furry_event_flag_free(context.event);
}

static bool lfrfid_cli_parse_args(FurryString* args, ProtocolDict* dict, ProtocolId* protocol) {
    bool result = false;
    FurryString *protocol_name, *data_text;
    protocol_name = furry_string_alloc();
    data_text = furry_string_alloc();
    ...
    do {
        ...
        // check protocol arg
        *protocol = protocol_dict_get_protocol_by_name(dict, furry_string_get_cstr(protocol_name));
        if(*protocol == PROTOCOL_NO) {
            printf(
                "Unknown protocol: %s\r\n"
                "Available protocols:\r\n",
                furry_string_get_cstr(protocol_name));
    ...
    free(data);
    furry_string_free(protocol_name);
    furry_string_free(data_text);
    return result;
}

static void lfrfid_cli_write_callback(LFRFIDWorkerWriteResult result, void* ctx) {
    furry_assert(ctx);
    FurryEventFlag* events = ctx;
    furry_event_flag_set(events, 1 << result);
}

static void lfrfid_cli_write(Cli* cli, FurryString* args) {
    ProtocolDict* dict = protocol_dict_alloc(lfrfid_protocols, LFRFIDProtocolMax);
    ProtocolId protocol;
    ...
    LFRFIDWorker* worker = lfrfid_worker_alloc(dict);
    FurryEventFlag* event = furry_event_flag_alloc();
    ...
    while(!cli_cmd_interrupt_received(cli)) {
        uint32_t flags = furry_event_flag_wait(event, available_flags, FurryFlagWaitAny, 100);
        if(flags != (unsigned)FurryFlagErrorTimeout) {
            if(FURRY_BIT(flags, LFRFIDWorkerWriteOK)) {
                printf("Written!\r\n");
                break;
            }

            if(FURRY_BIT(flags, LFRFIDWorkerWriteProtocolCannotBeWritten)) {
                printf("This protocol cannot be written.\r\n");
                break;
            }

            if(FURRY_BIT(flags, LFRFIDWorkerWriteFobCannotBeWritten)) {
    ...
    lfrfid_worker_free(worker);
    protocol_dict_free(dict);
    furry_event_flag_free(event);
}

static void lfrfid_cli_emulate(Cli* cli, FurryString* args) {
    ProtocolDict* dict = protocol_dict_alloc(lfrfid_protocols, LFRFIDProtocolMax);
    ProtocolId protocol;
...
static void lfrfid_cli_raw_analyze(Cli* cli, FurryString* args) {
    UNUSED(cli);
    FurryString *filepath, *info_string;
    filepath = furry_string_alloc();
    info_string = furry_string_alloc();
    ...
    do {
        ...
        if(!lfrfid_raw_file_open_read(file, furry_string_get_cstr(filepath))) {
        ...
        while(!file_end) {
            ...
            if(lfrfid_raw_file_read_pair(file, &duration, &pulse, &file_end)) {
                ...
                furry_string_printf(info_string, "[%lu %lu]", pulse, duration);
                printf("%-16s", furry_string_get_cstr(info_string));
                furry_string_printf(info_string, "[%lu %lu]", pulse, duration - pulse);
                printf("%-16s", furry_string_get_cstr(info_string));
        ...
        if(total_protocol != PROTOCOL_NO) {
            ...
            printf("%s\r\n", furry_string_get_cstr(info_string));
    ...
    furry_string_free(filepath);
    furry_string_free(info_string);
...
static void lfrfid_cli_raw_read_callback(LFRFIDWorkerReadRawResult result, void* context) {
    ...
    FurryEventFlag* event = context;
    furry_event_flag_set(event, 1 << result);
}

static void lfrfid_cli_raw_read(Cli* cli, FurryString* args) {
    UNUSED(cli);

    FurryString *filepath, *type_string;
    filepath = furry_string_alloc();
    type_string = furry_string_alloc();
    LFRFIDWorkerReadType type = LFRFIDWorkerReadTypeAuto;
    ...
    do {
        ...
        lfrfid_worker_read_raw_start(
            worker, furry_string_get_cstr(filepath), type, lfrfid_cli_raw_read_callback, event);
        while(true) {
            uint32_t flags = furry_event_flag_wait(event, available_flags, FurryFlagWaitAny, 100);

            if(flags != (unsigned)FurryFlagErrorTimeout) {
                if(FURRY_BIT(flags, LFRFIDWorkerReadRawFileError)) {
                    printf("File is not RFID raw file\r\n");
                    break;
                }

                if(FURRY_BIT(flags, LFRFIDWorkerReadRawOverrun)) {
                    if(!overrun) {
                        printf("Overrun\r\n");
                        overrun = true;
                    }
...
static void lfrfid_cli_raw_emulate(Cli* cli, FurryString* args) {
    UNUSED(cli);

    FurryString* filepath;
    filepath = furry_string_alloc();
    Storage* storage = furry_record_open(RECORD_STORAGE);
    ...
    do {
        ...
        if(!storage_file_exists(storage, furry_string_get_cstr(filepath))) {
            printf("File not found: \"%s\"\r\n", furry_string_get_cstr(filepath));
            break;
        }
        ...
        lfrfid_worker_emulate_raw_start(
            worker, furry_string_get_cstr(filepath), lfrfid_cli_raw_emulate_callback, event);
        while(true) {
            uint32_t flags = furry_event_flag_wait(event, available_flags, FurryFlagWaitAny, 100);

            if(flags != (unsigned)FurryFlagErrorTimeout) {
                if(FURRY_BIT(flags, LFRFIDWorkerEmulateRawFileError)) {
                    printf("File is not RFID raw file\r\n");
                    break;
                }

                if(FURRY_BIT(flags, LFRFIDWorkerEmulateRawOverrun)) {
                    if(!overrun) {
                        printf("Overrun\r\n");
                        overrun = true;
                    }
...
static void lfrfid_cli(Cli* cli, FurryString* args, void* context) {
    ...
    } else if(furry_string_cmp_str(cmd, "write") == 0) {
        lfrfid_cli_write(cli, args);
    } else if(furry_string_cmp_str(cmd, "emulate") == 0) {
        lfrfid_cli_emulate(cli, args);
    } else if(furry_string_cmp_str(cmd, "raw_read") == 0) {
        lfrfid_cli_raw_read(cli, args);
    } else if(furry_string_cmp_str(cmd, "raw_emulate") == 0) {
        lfrfid_cli_raw_emulate(cli, args);
    } else if(furry_string_cmp_str(cmd, "raw_analyze") == 0) {
