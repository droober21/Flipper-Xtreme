...
void lfrfid_scene_read_success_on_enter(void* context) {
    ...
    widget_add_string_element(
        widget, 16, 3, AlignLeft, AlignTop, FontPrimary, furry_string_get_cstr(tmp_string));

    furry_string_reset(tmp_string);
    size_t size = protocol_dict_get_data_size(app->dict, app->protocol_id);
    uint8_t* data = (uint8_t*)malloc(size);
    protocol_dict_get_data(app->dict, app->protocol_id, data, size);
    for(uint8_t i = 0; i < size; i++) {
        if(i >= 9) {
            furry_string_cat_printf(tmp_string, "..");
            break;
        } else {
            if(i != 0) {
                furry_string_cat_printf(tmp_string, ":");
            }
            furry_string_cat_printf(tmp_string, "%02X", data[i]);
        }
    }
    free(data);

    FurryString* render_data;
    render_data = furry_string_alloc();
    protocol_dict_render_brief_data(app->dict, render_data, app->protocol_id);
    furry_string_cat_printf(tmp_string, "\r\n%s", furry_string_get_cstr(render_data));
    furry_string_free(render_data);

    widget_add_string_multiline_element(
        widget, 0, 16, AlignLeft, AlignTop, FontSecondary, furry_string_get_cstr(tmp_string));
