...
static void nfc_cli_detect(Cli* cli, FurryString* args) {
    UNUSED(args);
    // Check if nfc worker is not busy
    if(furry_hal_nfc_is_busy()) {
        printf("Nfc is busy\r\n");
        return;
    }

    FurryHalNfcDevData dev_data = {};
    bool cmd_exit = false;
    furry_hal_nfc_exit_sleep();
    printf("Detecting nfc...\r\nPress Ctrl+C to abort\r\n");
    while(!cmd_exit) {
        cmd_exit |= cli_cmd_interrupt_received(cli);
        if(furry_hal_nfc_detect(&dev_data, 400)) {
            if(dev_data.type == FurryHalNfcTypeA) {
                ...
                printf("\r\n");
                break;
            } else if(dev_data.type == FurryHalNfcTypeF) {
                printf("IDm:");
...
static void nfc_cli_emulate(Cli* cli, FurryString* args) {
    UNUSED(args);
    // Check if nfc worker is not busy
    if(furry_hal_nfc_is_busy()) {
        printf("Nfc is busy\r\n");
...
static void nfc_cli_field(Cli* cli, FurryString* args) {
    UNUSED(args);
    // Check if nfc worker is not busy
    if(furry_hal_nfc_is_busy()) {
        printf("Nfc is busy\r\n");
    ...
    furry_hal_nfc_field_off();
    furry_hal_nfc_sleep();
}

static void nfc_cli_apdu(Cli* cli, FurryString* args) {
    UNUSED(cli);
    if(furry_hal_nfc_is_busy()) {
        printf("Nfc is busy\r\n");
        return;
...
static void nfc_cli(Cli* cli, FurryString* args, void* context) {
    UNUSED(context);
    FurryString* cmd;
    ...
    do {
        ...
        if(furry_string_cmp_str(cmd, "detect") == 0) {
            nfc_cli_detect(cli, args);
            break;
        }
        if(furry_string_cmp_str(cmd, "emulate") == 0) {
            nfc_cli_emulate(cli, args);
            break;
        }

        if(furry_string_cmp_str(cmd, "apdu") == 0) {
            nfc_cli_apdu(cli, args);
            break;
        }

        if(furry_hal_rtc_is_flag_set(FurryHalRtcFlagDebug)) {
            if(furry_string_cmp_str(cmd, "field") == 0) {
