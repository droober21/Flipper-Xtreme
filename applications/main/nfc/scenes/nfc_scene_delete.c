...
void nfc_scene_delete_on_enter(void* context) {
    ...
    furry_string_printf(temp_str, "\e#Delete %s?\e#", nfc->dev->dev_name);
    widget_add_text_box_element(
        nfc->widget, 0, 0, 128, 23, AlignCenter, AlignCenter, furry_string_get_cstr(temp_str), false);
    ...
    widget_add_string_element(
        nfc->widget, 64, 24, AlignCenter, AlignTop, FontSecondary, furry_string_get_cstr(temp_str));

    NfcProtocol protocol = nfc->dev->dev_data.protocol;
    const char* nfc_type = "NFC-A";

    if(protocol == NfcDeviceProtocolEMV) {
        furry_string_set(temp_str, "EMV bank card");
    } else if(protocol == NfcDeviceProtocolMifareUl) {
        furry_string_set(temp_str, nfc_mf_ul_type(nfc->dev->dev_data.mf_ul_data.type, true));
    } else if(protocol == NfcDeviceProtocolMifareClassic) {
        furry_string_set(temp_str, nfc_mf_classic_type(nfc->dev->dev_data.mf_classic_data.type));
    } else if(protocol == NfcDeviceProtocolMifareDesfire) {
        furry_string_set(temp_str, "MIFARE DESFire");
    } else if(protocol == NfcDeviceProtocolNfcV) {
        furry_string_set(temp_str, "ISO15693 tag");
        nfc_type = "NFC-V";
    } else if(protocol == NfcDeviceProtocolFelica) {
        furry_string_set(temp_str, "FeliCa");
    } else {
        furry_string_set(temp_str, "Unknown ISO tag");
    }
    widget_add_string_element(
        nfc->widget, 64, 34, AlignCenter, AlignTop, FontSecondary, furry_string_get_cstr(temp_str));
