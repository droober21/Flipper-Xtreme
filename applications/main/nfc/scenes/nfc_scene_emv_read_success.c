...
void nfc_scene_emv_read_success_on_enter(void* context) {
    ...
    if(emv_data->name[0] != '\0') {
        ...
        temp_str = furry_string_alloc_printf("\e#Unknown Bank Card\n");
    }
    if(emv_data->number_len) {
        ...
        furry_string_trim(temp_str);
    } else if(emv_data->aid_len) {
        furry_string_cat_printf(temp_str, "Can't parse data from app\n");
        // Parse AID name
        FurryString* aid_name;
        aid_name = furry_string_alloc();
        if(nfc_emv_parser_get_aid_name(
               nfc->dev->storage, emv_data->aid, emv_data->aid_len, aid_name)) {
            furry_string_cat_printf(temp_str, "AID: %s", furry_string_get_cstr(aid_name));
        } else {
            furry_string_cat_printf(temp_str, "AID: ");
            for(uint8_t i = 0; i < emv_data->aid_len; i++) {
                furry_string_cat_printf(temp_str, "%02X", emv_data->aid[i]);
            }
        }
        furry_string_free(aid_name);
    }
    ...
    if((emv_data->currency_code)) {
        ...
        currency_name = furry_string_alloc();
        if(nfc_emv_parser_get_currency_name(
               nfc->dev->storage, emv_data->currency_code, currency_name)) {
            furry_string_cat_printf(temp_str, "\nCur: %s  ", furry_string_get_cstr(currency_name));
        }
        furry_string_free(currency_name);
    }
    // Parse country code
    if((emv_data->country_code)) {
        FurryString* country_name;
        country_name = furry_string_alloc();
        if(nfc_emv_parser_get_country_name(
               nfc->dev->storage, emv_data->country_code, country_name)) {
            furry_string_cat_printf(temp_str, "Reg: %s", furry_string_get_cstr(country_name));
        }
        furry_string_free(country_name);
    }

    notification_message_block(nfc->notifications, &sequence_set_green_255);

    widget_add_text_scroll_element(nfc->widget, 0, 0, 128, 52, furry_string_get_cstr(temp_str));
