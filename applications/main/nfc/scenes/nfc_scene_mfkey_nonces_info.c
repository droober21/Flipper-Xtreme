...
void nfc_scene_mfkey_nonces_info_on_enter(void* context) {
    ...
    widget_add_text_scroll_element(nfc->widget, 0, 22, 128, 42, furry_string_get_cstr(temp_str));
    furry_string_printf(temp_str, "Nonce pairs saved: %d", nonces_saved);
    widget_add_string_element(
        nfc->widget, 0, 0, AlignLeft, AlignTop, FontPrimary, furry_string_get_cstr(temp_str));
    widget_add_string_element(
