...
static void subghz_frequency_analyzer_worker_load_registers(const uint8_t data[][2]) {
    ...
    size_t i = 0;
    while(data[i][0]) {
        cc1101_write_reg(furry_hal_subghz.spi_bus_handle, data[i][0], data[i][1]);
        i++;
    }
...
static int32_t subghz_frequency_analyzer_worker_thread(void* context) {
    ...
    cc1101_write_reg(furry_hal_subghz.spi_bus_handle, CC1101_MDMCFG3,
                     0b01111111); // symbol rate
    cc1101_write_reg(
        furry_hal_subghz.spi_bus_handle,
        CC1101_AGCCTRL2,
    ...
    while(instance->worker_running) {
        ...
        for(size_t i = 0; i < subghz_setting_get_frequency_count(instance->setting); i++) {
            ...
            if(furry_hal_subghz_is_frequency_valid(current_frequency) &&
               ...
               !((furry_hal_subghz.radio_type == SubGhzRadioExternal) &&
                ...
                frequency =
                    cc1101_set_frequency(furry_hal_subghz.spi_bus_handle, current_frequency);

                cc1101_calibrate(furry_hal_subghz.spi_bus_handle);
                do {
                    status = cc1101_get_status(furry_hal_subghz.spi_bus_handle);
        ...
        if(frequency_rssi.rssi_coarse > instance->trigger_level) {
            ...
            for(uint32_t i = frequency_rssi.frequency_coarse - 300000;
                ...
                i += 20000) {
                if(furry_hal_subghz_is_frequency_valid(i)) {
                    furry_hal_spi_acquire(furry_hal_subghz.spi_bus_handle);
                    cc1101_switch_to_idle(furry_hal_subghz.spi_bus_handle);
                    frequency = cc1101_set_frequency(furry_hal_subghz.spi_bus_handle, i);
                    ...
                    FURRY_LOG_T(TAG, "#:%lu:%f", frequency, (double)rssi);
...
SubGhzFrequencyAnalyzerWorker* subghz_frequency_analyzer_worker_alloc(void* context) {
    ...
    furry_thread_set_name(instance->thread, "SubGhzFAWorker");
    furry_thread_set_stack_size(instance->thread, 2048);
    furry_thread_set_context(instance->thread, instance);
    furry_thread_set_callback(instance->thread, subghz_frequency_analyzer_worker_thread);
