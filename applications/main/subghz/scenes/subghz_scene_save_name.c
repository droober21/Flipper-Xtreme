...
void subghz_scene_save_name_on_enter(void* context) {
    ...
    FurryString* dir_name;
    file_name = furry_string_alloc();
    dir_name = furry_string_alloc();

    if(!subghz_path_is_file(subghz->file_path)) {
        char file_name_buf[SUBGHZ_MAX_LEN_NAME] = {0};
        if(furry_hal_subghz_get_timestamp_file_names()) {
            ...
            furry_string_set(file_name, file_name_buf);
        }
        ...
        path_extract_dirname(furry_string_get_cstr(subghz->file_path), dir_name);
        ...
        furry_string_set(subghz->file_path, dir_name);
    }

    strncpy(subghz->file_name_tmp, furry_string_get_cstr(file_name), SUBGHZ_MAX_LEN_NAME);
    ...
    ValidatorIsFile* validator_is_file = validator_is_file_alloc_init(
        furry_string_get_cstr(subghz->file_path), SUBGHZ_APP_EXTENSION, "");
    text_input_set_validator(text_input, validator_is_file_callback, validator_is_file);

    furry_string_free(file_name);
    furry_string_free(dir_name);
    ...
    if(event.type == SceneManagerEventTypeBack) {
        if(!(strcmp(subghz->file_name_tmp, "") == 0) ||
           scene_manager_get_scene_state(subghz->scene_manager, SubGhzSceneReadRAW) !=
            ...
            if(!subghz->in_decoder_scene) {
                furry_string_set(subghz->file_path, subghz->file_path_tmp);
            }
        }
        ...
        if(event.event == SubGhzCustomEventSceneSaveName) {
            if(strcmp(subghz->file_name_tmp, "") != 0) {
                ...
                if(scene_manager_get_scene_state(subghz->scene_manager, SubGhzSceneReadRAW) !=
                   SubGhzCustomEventManagerNoSet) {
                    subghz_protocol_raw_gen_fff_data(
                        subghz->txrx->fff_data, furry_string_get_cstr(subghz->file_path));
                    scene_manager_set_scene_state(
                ...
                furry_string_set(subghz->error_str, "No name file");
