...
void subghz_cli_command_tx_carrier(Cli* cli, FurryString* args, void* context) {
    UNUSED(context);
    uint32_t frequency = 433920000;

    if(furry_string_size(args)) {
        int ret = sscanf(furry_string_get_cstr(args), "%lu", &frequency);
        if(ret != 1) {
            printf("sscanf returned %d, frequency: %lu\r\n", ret, frequency);
            cli_print_usage("subghz tx_carrier", "<Frequency: in Hz>", furry_string_get_cstr(args));
            return;
        }
...
void subghz_cli_command_rx_carrier(Cli* cli, FurryString* args, void* context) {
    UNUSED(context);
    uint32_t frequency = 433920000;
    ...
    if(furry_string_size(args)) {
        ...
        if(ret != 1) {
            printf("sscanf returned %d, frequency: %lu\r\n", ret, frequency);
            cli_print_usage("subghz rx_carrier", "<Frequency: in Hz>", furry_string_get_cstr(args));
            return;
        }
        if(!furry_hal_subghz_is_frequency_valid(frequency)) {
            printf(
    ...
    while(!cli_cmd_interrupt_received(cli)) {
        furry_delay_ms(250);
        printf("RSSI: %03.1fdbm\r", (double)furry_hal_subghz_get_rssi());
        fflush(stdout);
    }
...
void subghz_cli_command_tx(Cli* cli, FurryString* args, void* context) {
    UNUSED(context);
    uint32_t frequency = 433920000;
    ...
    if(furry_string_size(args)) {
        int ret =
            sscanf(furry_string_get_cstr(args), "%lx %lu %lu %lu", &key, &frequency, &te, &repeat);
        if(ret != 4) {
            printf(
    ...
    FurryString* flipper_format_string = furry_string_alloc_printf(
        "Protocol: Princeton\n"
        "Bit: 24\n"
    ...
    stream_clean(stream);
    stream_write_cstring(stream, furry_string_get_cstr(flipper_format_string));
    ...
    while(!(furry_hal_subghz_is_async_tx_complete() || cli_cmd_interrupt_received(cli))) {
        printf(".");
        fflush(stdout);
        furry_delay_ms(333);
    }
...
static void subghz_cli_command_rx_callback(
    ...
    printf("%s", furry_string_get_cstr(text));
    furry_string_free(text);
}

void subghz_cli_command_rx(Cli* cli, FurryString* args, void* context) {
    UNUSED(context);
    uint32_t frequency = 433920000;
    ...
    if(furry_string_size(args)) {
        ...
        if(ret != 1) {
            printf("sscanf returned %d, frequency: %lu\r\n", ret, frequency);
            cli_print_usage("subghz rx", "<Frequency: in Hz>", furry_string_get_cstr(args));
            return;
        }
        if(!furry_hal_subghz_is_frequency_valid(frequency)) {
            printf(
    ...
    instance->stream =
        furry_stream_buffer_alloc(sizeof(LevelDuration) * 1024, sizeof(LevelDuration));
    furry_check(instance->stream);
    ...
    furry_hal_subghz_start_async_rx(subghz_cli_command_rx_capture_callback, instance);
...
void subghz_cli_command_rx_raw(Cli* cli, FurryString* args, void* context) {
    UNUSED(context);
    uint32_t frequency = 433920000;
    ...
    // Configure radio
    furry_hal_subghz_reset();
    furry_hal_subghz_load_preset(FurryHalSubGhzPresetOok270Async);
    frequency = furry_hal_subghz_set_frequency_and_path(frequency);
    furry_hal_gpio_init(furry_hal_subghz.cc1101_g0_pin, GpioModeInput, GpioPullNo, GpioSpeedLow);
...
void subghz_cli_command_decode_raw(Cli* cli, FurryString* args, void* context) {
    UNUSED(context);
    FurryString* file_name;
    file_name = furry_string_alloc();
    furry_string_set(file_name, ANY_PATH("subghz/test.sub"));
    ...
    do {
        if(furry_string_size(args)) {
            if(!args_read_string_and_trim(args, file_name)) {
                cli_print_usage(
                    "subghz decode_raw", "<file_name: path_RAW_file>", furry_string_get_cstr(args));
                break;
            }
        }
        ...
        if(!strcmp(furry_string_get_cstr(temp_str), SUBGHZ_RAW_FILE_TYPE) &&
    ...
    if(check_file) {
        ...
        if(subghz_file_encoder_worker_start(file_worker_encoder, furry_string_get_cstr(file_name))) {
        ...
        LevelDuration level_duration;
        while(!cli_cmd_interrupt_received(cli)) {
            furry_delay_us(500); //you need to have time to read from the file from the SD card
...
static void subghz_cli_command_encrypt_keeloq(Cli* cli, FurryString* args) {
    UNUSED(cli);
    uint8_t iv[16];

    FurryString* source;
    FurryString* destination;
    source = furry_string_alloc();
    destination = furry_string_alloc();
    ...
    do {
        ...
        if(!subghz_keystore_load(keystore, furry_string_get_cstr(source))) {
            printf("Failed to load Keystore");
            break;
        }

        if(!subghz_keystore_save(keystore, furry_string_get_cstr(destination), iv)) {
    ...
    furry_string_free(destination);
    furry_string_free(source);
}

static void subghz_cli_command_encrypt_raw(Cli* cli, FurryString* args) {
    UNUSED(cli);
    uint8_t iv[16];
    ...
    do {
        ...
        if(!subghz_keystore_raw_encrypted_save(
               furry_string_get_cstr(source), furry_string_get_cstr(destination), iv)) {
            printf("Failed to save Keystore");
            break;
        }
...
static void subghz_cli_command_chat(Cli* cli, FurryString* args, void* context) {
    UNUSED(context);
    uint32_t frequency = 433920000;
    ...
    if(furry_string_size(args)) {
        ...
        if(ret != 1) {
            printf("sscanf returned %d, frequency: %lu\r\n", ret, frequency);
            cli_print_usage("subghz chat", "<Frequency: in Hz>", furry_string_get_cstr(args));
            return;
        }
        if(!furry_hal_subghz_is_frequency_valid(frequency)) {
            printf(
    ...
    FurryString* input;
    input = furry_string_alloc();
    FurryString* name;
    name = furry_string_alloc();
    FurryString* output;
    output = furry_string_alloc();
    FurryString* sysmsg;
    sysmsg = furry_string_alloc();
    bool exit = false;
    SubGhzChatEvent chat_event;

    NotificationApp* notification = furry_record_open(RECORD_NOTIFICATION);

    furry_string_printf(name, "\033[0;33m%s\033[0m: ", furry_hal_version_get_name_ptr());
    furry_string_set(input, name);
    printf("%s", furry_string_get_cstr(input));
    fflush(stdout);
    ...
    while(!exit) {
        ...
        case SubGhzChatEventInputData:
            if(chat_event.c == CliSymbolAsciiETX) {
                ...
                size_t len = furry_string_utf8_length(input);
                if(len > furry_string_utf8_length(name)) {
                    printf("%s", "\e[D\e[1P");
                    fflush(stdout);
                    //delete 1 char UTF
                    const char* str = furry_string_get_cstr(input);
                    size_t size = 0;
                    FurryStringUTF8State s = FurryStringUTF8StateStarting;
                    FurryStringUnicodeValue u = 0;
                    furry_string_reset(sysmsg);
                    while(*str) {
                        furry_string_utf8_decode(*str, &s, &u);
                        if((s == FurryStringUTF8StateError) || s == FurryStringUTF8StateStarting) {
                            furry_string_utf8_push(sysmsg, u);
                            if(++size >= len - 1) break;
                            s = FurryStringUTF8StateStarting;
                        }
                        str++;
                    }
                    furry_string_set(input, sysmsg);
                }
            } else if(chat_event.c == CliSymbolAsciiCR) {
                printf("\r\n");
                furry_string_push_back(input, '\r');
                furry_string_push_back(input, '\n');
                while(!subghz_chat_worker_write(
                    subghz_chat,
                    (uint8_t*)furry_string_get_cstr(input),
                    strlen(furry_string_get_cstr(input)))) {
                    furry_delay_ms(10);
                }

                furry_string_printf(input, "%s", furry_string_get_cstr(name));
                printf("%s", furry_string_get_cstr(input));
                fflush(stdout);
                ...
                fflush(stdout);
                furry_string_push_back(input, chat_event.c);
                break;
            case SubGhzChatEventRXData:
                do {
                    ...
                    for(size_t i = 0; i < len; i++) {
                        furry_string_push_back(output, message[i]);
                        if(message[i] == '\n') {
                            printf("\r");
                            for(uint8_t i = 0; i < 80; i++) {
                                printf(" ");
                            }
                            printf("\r %s", furry_string_get_cstr(output));
                            printf("%s", furry_string_get_cstr(input));
                            fflush(stdout);
                            furry_string_reset(output);
                        }
                    }
            ...
            case SubGhzChatEventUserEntrance:
                furry_string_printf(
                    ...
                    furry_hal_version_get_name_ptr());
                subghz_chat_worker_write(
                    subghz_chat,
                    (uint8_t*)furry_string_get_cstr(sysmsg),
                    strlen(furry_string_get_cstr(sysmsg)));
                break;
            case SubGhzChatEventUserExit:
                ...
                break;
            default:
                FURRY_LOG_W("SubGhzChat", "Error event");
                break;
            }
        }
    ...
    furry_string_free(input);
    furry_string_free(name);
    furry_string_free(output);
    furry_string_free(sysmsg);
    furry_hal_power_suppress_charge_exit();
...
static void subghz_cli_command(Cli* cli, FurryString* args, void* context) {
    FurryString* cmd;
    ...
    do {
        ...
        if(furry_string_cmp_str(cmd, "chat") == 0) {
            subghz_cli_command_chat(cli, args, NULL);
            break;
        }

        if(furry_string_cmp_str(cmd, "tx") == 0) {
            subghz_cli_command_tx(cli, args, context);
            break;
        }

        if(furry_string_cmp_str(cmd, "rx") == 0) {
            subghz_cli_command_rx(cli, args, context);
            break;
        }

        if(furry_string_cmp_str(cmd, "rx_raw") == 0) {
            subghz_cli_command_rx_raw(cli, args, context);
            break;
        }

        if(furry_string_cmp_str(cmd, "decode_raw") == 0) {
            subghz_cli_command_decode_raw(cli, args, context);
            break;
        }

        if(furry_hal_rtc_is_flag_set(FurryHalRtcFlagDebug)) {
            if(furry_string_cmp_str(cmd, "encrypt_keeloq") == 0) {
                subghz_cli_command_encrypt_keeloq(cli, args);
                break;
            }

            if(furry_string_cmp_str(cmd, "encrypt_raw") == 0) {
                subghz_cli_command_encrypt_raw(cli, args);
                break;
            }

            if(furry_string_cmp_str(cmd, "tx_carrier") == 0) {
                subghz_cli_command_tx_carrier(cli, args, context);
                break;
            }

            if(furry_string_cmp_str(cmd, "rx_carrier") == 0) {
