...
static void bt_cli_command_hci_info(Cli* cli, FurryString* args, void* context) {
    UNUSED(cli);
    UNUSED(args);
    UNUSED(context);
    FurryString* buffer;
    buffer = furry_string_alloc();
    furry_hal_bt_dump_state(buffer);
    printf("%s", furry_string_get_cstr(buffer));
    furry_string_free(buffer);
}

static void bt_cli_command_carrier_tx(Cli* cli, FurryString* args, void* context) {
    UNUSED(context);
    int channel = 0;
    int power = 0;
    ...
    do {
        ...
        furry_hal_bt_start_tone_tx(channel, 0x19 + power);

        while(!cli_cmd_interrupt_received(cli)) {
            furry_delay_ms(250);
        }
        furry_hal_bt_stop_tone_tx();
...
static void bt_cli_command_carrier_rx(Cli* cli, FurryString* args, void* context) {
    UNUSED(context);
    int channel = 0;
    ...
    do {
        ...
        furry_hal_bt_start_packet_rx(channel, 1);

        while(!cli_cmd_interrupt_received(cli)) {
            furry_delay_ms(250);
            printf("RSSI: %6.1f dB\r", (double)furry_hal_bt_get_rssi());
            fflush(stdout);
        }
...
static void bt_cli_command_packet_tx(Cli* cli, FurryString* args, void* context) {
    UNUSED(context);
    int channel = 0;
    int pattern = 0;
    int datarate = 1;
    ...
    do {
        ...
        printf(
            ...
            datarate);
        printf("Press CTRL+C to stop\r\n");
        furry_hal_bt_start_packet_tx(channel, pattern, datarate);

        while(!cli_cmd_interrupt_received(cli)) {
            furry_delay_ms(250);
        }
        furry_hal_bt_stop_packet_test();
        printf("Transmitted %lu packets", furry_hal_bt_get_transmitted_packets());
...
static void bt_cli_command_packet_rx(Cli* cli, FurryString* args, void* context) {
    UNUSED(context);
    int channel = 0;
    int datarate = 1;
    ...
    do {
        ...
        printf("Press CTRL+C to stop\r\n");
        furry_hal_bt_start_packet_rx(channel, datarate);

        while(!cli_cmd_interrupt_received(cli)) {
            furry_delay_ms(250);
            printf("RSSI: %03.1f dB\r", (double)furry_hal_bt_get_rssi());
            fflush(stdout);
        }
        uint16_t packets_received = furry_hal_bt_stop_packet_test();
...
static void bt_cli_print_usage() {
    ...
    if(furry_hal_rtc_is_flag_set(FurryHalRtcFlagDebug) && furry_hal_bt_is_testing_supported()) {
...
static void bt_cli(Cli* cli, FurryString* args, void* context) {
    UNUSED(context);
    furry_record_open(RECORD_BT);
    ...
    do {
        ...
        if(furry_string_cmp_str(cmd, "hci_info") == 0) {
        ...
        if(furry_hal_rtc_is_flag_set(FurryHalRtcFlagDebug) && furry_hal_bt_is_testing_supported()) {
            ...
            if(furry_string_cmp_str(cmd, "tx_packet") == 0) {
                bt_cli_command_packet_tx(cli, args, NULL);
                break;
            }
            if(furry_string_cmp_str(cmd, "rx_packet") == 0) {
