...
void crypto_cli_encrypt(Cli* cli, FurryString* args) {
    int key_slot = 0;
    ...
    do {
        ...
        while(cli_read(cli, (uint8_t*)&c, 1) == 1) {
            if(c == CliSymbolAsciiETX) {
                ...
                putc(c, stdout);
                fflush(stdout);
                furry_string_push_back(input, c);
            } else if(c == CliSymbolAsciiCR) {
                printf("\r\n");
                furry_string_cat(input, "\r\n");
            }
        }

        size_t size = furry_string_size(input);
        if(size > 0) {
            ...
            furry_string_reserve(input, size);
            uint8_t* output = malloc(size);
            if(!furry_hal_crypto_encrypt(
                   (const uint8_t*)furry_string_get_cstr(input), output, size)) {
...
void crypto_cli_decrypt(Cli* cli, FurryString* args) {
    int key_slot = 0;
    bool key_loaded = false;
    uint8_t iv[16];
    ...
    do {
        ...
        FurryString* hex_input;
        hex_input = furry_string_alloc();
        char c;
        while(cli_read(cli, (uint8_t*)&c, 1) == 1) {
            if(c == CliSymbolAsciiETX) {
                printf("\r\n");
                ...
                putc(c, stdout);
                fflush(stdout);
                furry_string_push_back(hex_input, c);
            } else if(c == CliSymbolAsciiCR) {
                printf("\r\n");
            }
        }

        furry_string_trim(hex_input);
        size_t hex_size = furry_string_size(hex_input);
        if(hex_size > 0 && hex_size % 2 == 0) {
            ...
            if(args_read_hex_bytes(hex_input, input, size)) {
                if(furry_hal_crypto_decrypt(input, output, size)) {
...
void crypto_cli_has_key(Cli* cli, FurryString* args) {
    UNUSED(cli);
    int key_slot = 0;
    uint8_t iv[16] = {0};
...
void crypto_cli_store_key(Cli* cli, FurryString* args) {
    UNUSED(cli);
    int key_slot = 0;
    int key_size = 0;
    FurryString* key_type;
    key_type = furry_string_alloc();
    ...
    do {
        ...
        if(furry_string_cmp_str(key_type, "master") == 0) {
            if(key_slot != 0) {
                printf("Master keyslot must be is 0");
                break;
            }
            key.type = FurryHalCryptoKeyTypeMaster;
        } else if(furry_string_cmp_str(key_type, "simple") == 0) {
            if(key_slot < 1 || key_slot > 99) {
                printf("Simple keyslot must be in range");
                break;
            }
            key.type = FurryHalCryptoKeyTypeSimple;
        } else if(furry_string_cmp_str(key_type, "encrypted") == 0) {
            key.type = FurryHalCryptoKeyTypeEncrypted;
            data_size += 12;
        } else {
            printf("Incorrect or missing key type, expected master, simple or encrypted");
            break;
        ...
        if(key_size == 128) {
            key.size = FurryHalCryptoKeySize128;
            data_size += 16;
        ...
        if(key_slot > 0) {
            uint8_t iv[16] = {0};
            if(key_slot > 1) {
                if(!furry_hal_crypto_store_load_key(key_slot - 1, iv)) {
                    printf(
    ...
    furry_string_free(key_type);
}

static void crypto_cli(Cli* cli, FurryString* args, void* context) {
    UNUSED(context);
    FurryString* cmd;
    ...
    do {
        ...
        if(furry_string_cmp_str(cmd, "encrypt") == 0) {
            crypto_cli_encrypt(cli, args);
            break;
        }

        if(furry_string_cmp_str(cmd, "decrypt") == 0) {
            crypto_cli_decrypt(cli, args);
            break;
        }

        if(furry_string_cmp_str(cmd, "has_key") == 0) {
            crypto_cli_has_key(cli, args);
            break;
        }

        if(furry_string_cmp_str(cmd, "store_key") == 0) {
