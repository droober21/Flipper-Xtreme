...
static bool
    widget_element_text_scroll_process_ctrl_symbols(TextScrollLineArray* line, FurryString* text) {
    bool processed = false;

    do {
        if(furry_string_get_char(text, 0) != '\e') break;
        char ctrl_symbol = furry_string_get_char(text, 1);
        if(ctrl_symbol == 'c') {
...
void widget_element_text_scroll_add_line(WidgetElement* element, TextScrollLineArray* line) {
    ...
    new_line.text = furry_string_alloc_set(line->text);
...
static void widget_element_text_scroll_fill_lines(Canvas* canvas, WidgetElement* element) {
    ...
    line_tmp.text = furry_string_alloc();
    ...
    while(!all_text_processed) {
        if(reached_new_line) {
            ...
            furry_string_reset(line_tmp.text);
        ...
        uint16_t char_i = 0;
        while(true) {
            char next_char = furry_string_get_char(model->text, char_i++);
            if(next_char == '\0') {
                furry_string_push_back(line_tmp.text, '\0');
                ...
                furry_string_push_back(line_tmp.text, '\0');
                widget_element_text_scroll_add_line(element, &line_tmp);
                furry_string_right(model->text, char_i);
                total_height += params->leading_default - params->height;
                ...
                if(line_width > model->width) {
                    furry_string_push_back(line_tmp.text, '\0');
                    widget_element_text_scroll_add_line(element, &line_tmp);
                    furry_string_right(model->text, char_i - 1);
                    furry_string_reset(line_tmp.text);
                    total_height += params->leading_default - params->height;
                    reached_new_line = false;
                    break;
                } else {
                    furry_string_push_back(line_tmp.text, next_char);
                }
            }
        }
    }

    furry_string_free(line_tmp.text);
}
...
static void widget_element_text_scroll_draw(Canvas* canvas, WidgetElement* element) {
    ...
    furry_mutex_acquire(element->model_mutex, FurryWaitForever);
    ...
    if(TextScrollLineArray_size(model->line_array)) {
        ...
        for(TextScrollLineArray_it(it, model->line_array); !TextScrollLineArray_end_p(it);
            ...
            canvas_draw_str_aligned(
                canvas, x, y, line->horizontal, AlignTop, furry_string_get_cstr(line->text));
    ...
    furry_mutex_release(element->model_mutex);
}
...
WidgetElement* widget_element_text_scroll_create(
    ...
    text_scroll->model_mutex = furry_mutex_alloc(FurryMutexTypeNormal);
