...
#define GPIO_Read(input_pin) (furry_hal_gpio_read(input_pin.pin->gpio) ^ (input_pin.pin->inverted))

static Input* input = NULL;

inline static void input_timer_start(FurryTimer* timer_id, uint32_t ticks) {
    TimerHandle_t hTimer = (TimerHandle_t)timer_id;
    furry_check(xTimerChangePeriod(hTimer, ticks, portMAX_DELAY) == pdPASS);
}

inline static void input_timer_stop(FurryTimer* timer_id) {
    TimerHandle_t hTimer = (TimerHandle_t)timer_id;
    ...
    while(xTimerIsTimerActive(hTimer) == pdTRUE) furry_delay_tick(1);
}
...
void input_isr(void* _ctx) {
    UNUSED(_ctx);
    furry_thread_flags_set(input->thread_id, INPUT_THREAD_FLAG_ISR);
}
...
int32_t input_srv(void* p) {
    ...
    input->thread_id = furry_thread_get_current_id();
    input->event_pubsub = furry_pubsub_alloc();
    furry_record_create(RECORD_INPUT_EVENTS, input->event_pubsub);

#if INPUT_DEBUG
    furry_hal_gpio_init_simple(&gpio_ext_pa4, GpioModeOutputPushPull);
#endif

#ifdef SRV_CLI
    input->cli = furry_record_open(RECORD_CLI);
    ...
    for(size_t i = 0; i < input_pins_count; i++) {
        furry_hal_gpio_add_int_callback(input_pins[i].gpio, input_isr, NULL);
        ...
        input->pin_states[i].press_timer = furry_timer_alloc(
            input_press_timer_callback, FurryTimerTypePeriodic, &input->pin_states[i]);
    ...
    while(1) {
        ...
        if(is_changing) {
#if INPUT_DEBUG
            furry_hal_gpio_write(&gpio_ext_pa4, 1);
#endif
            furry_delay_tick(1);
        } else {
#if INPUT_DEBUG
            furry_hal_gpio_write(&gpio_ext_pa4, 0);
#endif
            furry_thread_flags_wait(INPUT_THREAD_FLAG_ISR, FurryFlagWaitAny, FurryWaitForever);
        }
    }
