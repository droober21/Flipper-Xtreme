...
typedef struct {
    ...
    FurrySemaphore* terminate_semaphore;
} CliRpc;
...
static void rpc_cli_session_terminated_callback(void* context) {
    ...
    furry_semaphore_release(cli_rpc->terminate_semaphore);
}

void rpc_cli_command_start_session(Cli* cli, FurryString* args, void* context) {
    UNUSED(args);
    furry_assert(cli);
    furry_assert(context);
    Rpc* rpc = context;
    ...
    FURRY_LOG_D(TAG, "Free memory %lu", mem_before);
    ...
    cli_rpc.terminate_semaphore = furry_semaphore_alloc(1, 0);
    ...
    while(1) {
        ...
        if(size_received) {
            ...
            (void)fed_bytes;
            furry_assert(fed_bytes == size_received);
        }
    }
    ...
    furry_check(
        furry_semaphore_acquire(cli_rpc.terminate_semaphore, FurryWaitForever) == FurryStatusOk);

    furry_semaphore_free(cli_rpc.terminate_semaphore);
