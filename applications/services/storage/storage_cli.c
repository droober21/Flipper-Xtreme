...
static void storage_cli_info(Cli* cli, FurryString* path) {
    UNUSED(cli);
    Storage* api = furry_record_open(RECORD_STORAGE);

    if(furry_string_cmp_str(path, STORAGE_INT_PATH_PREFIX) == 0) {
        ...
        if(error != FSE_OK) {
            ...
            printf(
                "Label: %s\r\nType: LittleFS\r\n%luKiB total\r\n%luKiB free\r\n",
                furry_hal_version_get_name_ptr() ? furry_hal_version_get_name_ptr() : "Unknown",
                (uint32_t)(total_space / 1024),
                (uint32_t)(free_space / 1024));
        }
    } else if(furry_string_cmp_str(path, STORAGE_EXT_PATH_PREFIX) == 0) {
        SDInfo sd_info;
...
static void storage_cli_format(Cli* cli, FurryString* path) {
    if(furry_string_cmp_str(path, STORAGE_INT_PATH_PREFIX) == 0) {
...
static void storage_cli_list(Cli* cli, FurryString* path) {
    UNUSED(cli);
    if(furry_string_cmp_str(path, "/") == 0) {
        printf("\t[D] int\r\n");
        printf("\t[D] ext\r\n");
        printf("\t[D] any\r\n");
    } else {
        ...
        if(storage_dir_open(file, furry_string_get_cstr(path))) {
            FileInfo fileinfo;
...
static void storage_cli_tree(Cli* cli, FurryString* path) {
    if(furry_string_cmp_str(path, "/") == 0) {
        furry_string_set(path, STORAGE_INT_PATH_PREFIX);
        storage_cli_tree(cli, path);
        furry_string_set(path, STORAGE_EXT_PATH_PREFIX);
        storage_cli_tree(cli, path);
    } else {
        Storage* api = furry_record_open(RECORD_STORAGE);
        ...
        if(dir_walk_open(dir_walk, furry_string_get_cstr(path))) {
            FileInfo fileinfo;
            bool read_done = false;
...
static void storage_cli_read(Cli* cli, FurryString* path) {
    UNUSED(cli);
    Storage* api = furry_record_open(RECORD_STORAGE);
    File* file = storage_file_alloc(api);
...
static void storage_cli_write(Cli* cli, FurryString* path) {
    Storage* api = furry_record_open(RECORD_STORAGE);
    File* file = storage_file_alloc(api);
    ...
    if(storage_file_open(file, furry_string_get_cstr(path), FSAM_WRITE, FSOM_OPEN_APPEND)) {
...
static void storage_cli_read_chunks(Cli* cli, FurryString* path, FurryString* args) {
    Storage* api = furry_record_open(RECORD_STORAGE);
    File* file = storage_file_alloc(api);

    uint32_t buffer_size;
    int parsed_count = sscanf(furry_string_get_cstr(args), "%lu", &buffer_size);

    if(parsed_count != 1) {
        storage_cli_print_usage();
    } else if(storage_file_open(file, furry_string_get_cstr(path), FSAM_READ, FSOM_OPEN_EXISTING)) {
...
static void storage_cli_write_chunk(Cli* cli, FurryString* path, FurryString* args) {
    Storage* api = furry_record_open(RECORD_STORAGE);
    File* file = storage_file_alloc(api);
...
static void storage_cli_stat(Cli* cli, FurryString* path) {
    UNUSED(cli);
    Storage* api = furry_record_open(RECORD_STORAGE);
    ...
    if(furry_string_cmp_str(path, "/") == 0) {
        ...
        furry_string_cmp_str(path, STORAGE_EXT_PATH_PREFIX) == 0 ||
        furry_string_cmp_str(path, STORAGE_INT_PATH_PREFIX) == 0 ||
        furry_string_cmp_str(path, STORAGE_ANY_PATH_PREFIX) == 0) {
        uint64_t total_space;
        uint64_t free_space;
        FS_Error error =
            storage_common_fs_info(api, furry_string_get_cstr(path), &total_space, &free_space);
        ...
        FileInfo fileinfo;
        FS_Error error = storage_common_stat(api, furry_string_get_cstr(path), &fileinfo);
...
static void storage_cli_timestamp(Cli* cli, FurryString* path) {
    UNUSED(cli);
    Storage* api = furry_record_open(RECORD_STORAGE);

    uint32_t timestamp = 0;
    FS_Error error = storage_common_timestamp(api, furry_string_get_cstr(path), &timestamp);
...
static void storage_cli_copy(Cli* cli, FurryString* old_path, FurryString* args) {
    UNUSED(cli);
    Storage* api = furry_record_open(RECORD_STORAGE);
    FurryString* new_path;
    new_path = furry_string_alloc();

    if(!args_read_probably_quoted_string_and_trim(args, new_path)) {
        storage_cli_print_usage();
    } else {
        FS_Error error = storage_common_copy(
            api, furry_string_get_cstr(old_path), furry_string_get_cstr(new_path));
...
static void storage_cli_remove(Cli* cli, FurryString* path) {
    UNUSED(cli);
    Storage* api = furry_record_open(RECORD_STORAGE);
    FS_Error error = storage_common_remove(api, furry_string_get_cstr(path));
...
static void storage_cli_rename(Cli* cli, FurryString* old_path, FurryString* args) {
    UNUSED(cli);
    Storage* api = furry_record_open(RECORD_STORAGE);
    FurryString* new_path;
    new_path = furry_string_alloc();
...
static void storage_cli_mkdir(Cli* cli, FurryString* path) {
    UNUSED(cli);
    Storage* api = furry_record_open(RECORD_STORAGE);
    FS_Error error = storage_common_mkdir(api, furry_string_get_cstr(path));
...
static void storage_cli_md5(Cli* cli, FurryString* path) {
    UNUSED(cli);
    Storage* api = furry_record_open(RECORD_STORAGE);
    File* file = storage_file_alloc(api);
...
void storage_cli(Cli* cli, FurryString* args, void* context) {
    UNUSED(context);
    FurryString* cmd;
    FurryString* path;
    ...
    do {
        ...
        if(furry_string_cmp_str(cmd, "format") == 0) {
        ...
        if(furry_string_cmp_str(cmd, "tree") == 0) {
            storage_cli_tree(cli, path);
            break;
        }
        ...
        if(furry_string_cmp_str(cmd, "read_chunks") == 0) {
            storage_cli_read_chunks(cli, path, args);
            break;
        }

        if(furry_string_cmp_str(cmd, "write") == 0) {
            storage_cli_write(cli, path);
            break;
        }

        if(furry_string_cmp_str(cmd, "write_chunk") == 0) {
            storage_cli_write_chunk(cli, path, args);
            break;
        }

        if(furry_string_cmp_str(cmd, "copy") == 0) {
            storage_cli_copy(cli, path, args);
            break;
        }

        if(furry_string_cmp_str(cmd, "remove") == 0) {
            storage_cli_remove(cli, path);
            break;
        }

        if(furry_string_cmp_str(cmd, "rename") == 0) {
            storage_cli_rename(cli, path, args);
            break;
        }

        if(furry_string_cmp_str(cmd, "mkdir") == 0) {
            storage_cli_mkdir(cli, path);
            break;
        }

        if(furry_string_cmp_str(cmd, "md5") == 0) {
            storage_cli_md5(cli, path);
            break;
        }

        if(furry_string_cmp_str(cmd, "stat") == 0) {
            storage_cli_stat(cli, path);
            break;
        }

        if(furry_string_cmp_str(cmd, "timestamp") == 0) {
...
static void storage_cli_factory_reset(Cli* cli, FurryString* args, void* context) {
    UNUSED(args);
    UNUSED(context);
