...
static DialogMessageButton product_screen(DialogsApp* dialogs, DialogMessage* message) {
    ...
    FurryString* screen_header = furry_string_alloc_printf(
        "Product: %s\n"
        "Model: %s",
        furry_hal_version_get_model_name(),
        furry_hal_version_get_model_code());

    FurryString* screen_text = furry_string_alloc_printf(
        "FCC ID: %s\n"
        "IC: %s",
        furry_hal_version_get_fcc_id(),
        furry_hal_version_get_ic_id());

    dialog_message_set_header(
        message, furry_string_get_cstr(screen_header), 0, 0, AlignLeft, AlignTop);
    dialog_message_set_text(
        message, furry_string_get_cstr(screen_text), 0, 26, AlignLeft, AlignTop);
    result = dialog_message_show(dialogs, message);
    dialog_message_set_header(message, NULL, 0, 0, AlignLeft, AlignTop);
    dialog_message_set_text(message, NULL, 0, 0, AlignLeft, AlignTop);

    furry_string_free(screen_header);
    furry_string_free(screen_text);
...
static DialogMessageButton hw_version_screen(DialogsApp* dialogs, DialogMessage* message) {
    ...
    furry_string_cat_printf(buffer, "Serial Number:\n");
    const uint8_t* uid = furry_hal_version_uid();
    for(size_t i = 0; i < furry_hal_version_uid_size(); i++) {
        furry_string_cat_printf(buffer, "%02X", uid[i]);
    }

    dialog_message_set_header(message, "Hardware Info:", 0, 0, AlignLeft, AlignTop);
    dialog_message_set_text(message, furry_string_get_cstr(buffer), 0, 13, AlignLeft, AlignTop);
    result = dialog_message_show(dialogs, message);
    dialog_message_set_text(message, NULL, 0, 0, AlignLeft, AlignTop);
    dialog_message_set_header(message, NULL, 0, 0, AlignLeft, AlignTop);
...
static DialogMessageButton fw_version_screen(DialogsApp* dialogs, DialogMessage* message) {
    ...
    if(!ver) { //-V1051
        furry_string_cat_printf(buffer, "No info\n");
    } else {
        furry_string_cat_printf(
            buffer,
            "%s [%s]\n%s%s [%s] %s\n[%d] %s",
...
static void battery_info_draw_callback(Canvas* canvas, void* context) {
    ...
    char uptime[17];
    uint32_t sec = furry_get_tick() / furry_kernel_get_tick_frequency();
...
static bool battery_info_input_callback(InputEvent* event, void* context) {
    FurrySemaphore* semaphore = context;
...
int32_t about_settings_app(void* p) {
    ...
    while(screen_index > -2) {
        if(screen_index == -1) {
            ...
            furry_semaphore_acquire(semaphore, 2000);
        } else {
