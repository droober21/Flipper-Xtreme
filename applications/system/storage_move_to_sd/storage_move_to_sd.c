...
static StorageMoveToSd* storage_move_to_sd_alloc() {
    ...
    Storage* storage = furry_record_open(RECORD_STORAGE);
    app->sub = furry_pubsub_subscribe(
...
static void storage_move_to_sd_free(StorageMoveToSd* app) {
    Storage* storage = furry_record_open(RECORD_STORAGE);
    furry_pubsub_unsubscribe(storage_get_pubsub(storage), app->sub);
    furry_record_close(RECORD_STORAGE);
...
int32_t storage_move_to_sd_app(void* p) {
    ...
    if(storage_move_to_sd_check()) {
        ...
        FURRY_LOG_I(TAG, "Nothing to move");
    }
...
int32_t storage_move_to_sd_start(void* p) {
    ...
    furry_pubsub_subscribe(storage_get_pubsub(storage), storage_move_to_sd_mount_callback, NULL);
