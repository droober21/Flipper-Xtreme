...
static bool update_task_pre_update(UpdateTask* update_task) {
    bool success = false;
    FurryString* backup_file_path;
    backup_file_path = furry_string_alloc();
    path_concat(
        furry_string_get_cstr(update_task->update_path),
    ...
    if((success =
            lfs_backup_create(update_task->storage, furry_string_get_cstr(backup_file_path)))) {
        furry_hal_rtc_set_boot_mode(FurryHalRtcBootModeUpdate);
    }

    furry_string_free(backup_file_path);
    return success;
}
...
static void update_task_cleanup_resources(UpdateTask* update_task, const uint32_t n_tar_entries) {
    ...
    do {
        FURRY_LOG_D(TAG, "Cleaning up old manifest");
        if(!resource_manifest_reader_open(manifest_reader, EXT_PATH("Manifest"))) {
            FURRY_LOG_W(TAG, "No existing manifest");
            break;
        }
        ...
        while((entry_ptr = resource_manifest_reader_next(manifest_reader))) {
            if(entry_ptr->type == ResourceManifestEntryTypeFile) {
                ...
                path_concat(
                    STORAGE_EXT_PATH_PREFIX, furry_string_get_cstr(entry_ptr->name), file_path);
                FURRY_LOG_D(TAG, "Removing %s", furry_string_get_cstr(file_path));

                FS_Error result =
                    storage_common_remove(update_task->storage, furry_string_get_cstr(file_path));
        ...
        while((entry_ptr = resource_manifest_reader_previous(manifest_reader))) {
            if(entry_ptr->type == ResourceManifestEntryTypeDirectory) {
                ...
                FurryString* folder_path = furry_string_alloc();

                do {
                    path_concat(
                        STORAGE_EXT_PATH_PREFIX,
                        furry_string_get_cstr(entry_ptr->name),
                        folder_path);

                    FURRY_LOG_D(TAG, "Removing folder %s", furry_string_get_cstr(folder_path));
                    FS_Error result = storage_common_remove(
                        update_task->storage, furry_string_get_cstr(folder_path));
                    if(result != FSE_OK && result != FSE_EXIST) {
                        FURRY_LOG_E(
                            TAG,
                            "%s remove failed, cause %s",
...
static bool update_task_post_update(UpdateTask* update_task) {
    ...
    do {
        ...
        CHECK_RESULT(lfs_backup_unpack(update_task->storage, furry_string_get_cstr(file_path)));
        ...
        if(update_task->state.groups & UpdateTaskStageGroupResources) {
            ...
            path_concat(
                furry_string_get_cstr(update_task->update_path),
                furry_string_get_cstr(update_task->manifest->resource_bundle),
                file_path);

            tar_archive_set_file_callback(archive, update_task_resource_unpack_cb, &progress);
            CHECK_RESULT(
                tar_archive_open(archive, furry_string_get_cstr(file_path), TAR_OPEN_MODE_READ));
            ...
            if(storage_common_stat(update_task->storage, XFWFIRSTBOOT_FLAG_PATH, NULL) ==
               FSE_NOT_EXIST) {
                ...
                if(storage_common_stat(
                       update_task->storage, furry_string_get_cstr(tmp_path), NULL) != FSE_OK) {
                    furry_string_set(tmp_path, update_task->update_path);
                    path_append(
                        tmp_path, furry_string_get_cstr(update_task->manifest->splash_file));
                }
            } else {
                path_append(tmp_path, furry_string_get_cstr(update_task->manifest->splash_file));
            }
            if(storage_common_copy(
                   update_task->storage, furry_string_get_cstr(tmp_path), SLIDESHOW_FS_PATH) !=
               FSE_OK) {
...
int32_t update_task_worker_backup_restore(void* context) {
    ...
    FurryHalRtcBootMode boot_mode = update_task->boot_mode;
    if((boot_mode != FurryHalRtcBootModePreUpdate) && (boot_mode != FurryHalRtcBootModePostUpdate)) {
    ...
    do {
        ...
        if(boot_mode == FurryHalRtcBootModePreUpdate) {
            success = update_task_pre_update(update_task);
        } else if(boot_mode == FurryHalRtcBootModePostUpdate) { //-V547
