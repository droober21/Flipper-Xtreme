...
static FurryMutex* furry_hal_crypto_mutex = NULL;
static bool furry_hal_crypto_mode_init_done = false;
...
void furry_hal_crypto_init() {
    furry_hal_crypto_mutex = furry_mutex_alloc(FurryMutexTypeNormal);
    FURRY_LOG_I(TAG, "Init OK");
}

static bool furry_hal_crypto_generate_unique_keys(uint8_t start_slot, uint8_t end_slot) {
    FurryHalCryptoKey key;
    uint8_t key_data[32];
    FURRY_LOG_I(TAG, "Generating keys %u..%u", start_slot, end_slot);
    for(uint8_t slot = start_slot; slot <= end_slot; slot++) {
        key.type = FurryHalCryptoKeyTypeSimple;
        key.size = FurryHalCryptoKeySize256;
        key.data = key_data;
        furry_hal_random_fill_buf(key_data, 32);
        if(!furry_hal_crypto_store_add_key(&key, &slot)) {
            explicit_bzero(key_data, sizeof(key_data));
            FURRY_LOG_E(TAG, "Error writing key to slot %u", slot);
            return false;
        }
        explicit_bzero(key_data, sizeof(key_data));
    }
...
bool furry_hal_crypto_verify_key(uint8_t key_slot) {
    ...
    furry_hal_crypto_verify_enclave(&keys_nb, &valid_keys_nb);
    if(key_slot <= ENCLAVE_FACTORY_KEY_SLOTS) { // It's a factory key
        ...
        for(uint8_t i = key_slot; i > ENCLAVE_FACTORY_KEY_SLOTS; i--) {
            if(furry_hal_crypto_store_load_key(i, empty_iv)) {
                last_valid_slot = i;
                furry_hal_crypto_store_unload_key(i);
                break;
            }
        }
        if(last_valid_slot == key_slot)
            ...
            return furry_hal_crypto_generate_unique_keys(last_valid_slot + 1, key_slot);
    }
    return true;
}

bool furry_hal_crypto_verify_enclave(uint8_t* keys_nb, uint8_t* valid_keys_nb) {
    furry_assert(keys_nb);
    furry_assert(valid_keys_nb);
    uint8_t keys = 0;
    ...
    for(size_t key_slot = 0; key_slot < ENCLAVE_FACTORY_KEY_SLOTS; key_slot++) {
        if(furry_hal_crypto_store_load_key(key_slot + 1, enclave_signature_iv[key_slot])) {
            keys++;
            ...
            furry_hal_crypto_store_unload_key(key_slot + 1);
        }
    }
    *keys_nb = keys;
...
bool furry_hal_crypto_store_add_key(FurryHalCryptoKey* key, uint8_t* slot) {
    ...
    furry_check(furry_mutex_acquire(furry_hal_crypto_mutex, FurryWaitForever) == FurryStatusOk);
    ...
    if(key->type == FurryHalCryptoKeyTypeMaster) {
        pParam.KeyType = KEYTYPE_MASTER;
    } else if(key->type == FurryHalCryptoKeyTypeSimple) {
        pParam.KeyType = KEYTYPE_SIMPLE;
    } else if(key->type == FurryHalCryptoKeyTypeEncrypted) {
        pParam.KeyType = KEYTYPE_ENCRYPTED;
        key_data_size += 12;
    } else {
        furry_crash("Incorrect key type");
    }

    if(key->size == FurryHalCryptoKeySize128) {
        pParam.KeySize = KEYSIZE_16;
        key_data_size += 16;
    } else if(key->size == FurryHalCryptoKeySize256) {
        pParam.KeySize = KEYSIZE_32;
        key_data_size += 32;
    } else {
        furry_crash("Incorrect key size");
    }
    ...
    furry_check(furry_mutex_release(furry_hal_crypto_mutex) == FurryStatusOk);
...
static bool crypto_process_block(uint32_t* in, uint32_t* out, uint8_t blk_len) {
    furry_check((blk_len <= 4) && (blk_len > 0));
...
bool furry_hal_crypto_store_load_key(uint8_t slot, const uint8_t* iv) {
    furry_assert(slot > 0 && slot <= 100);
    furry_assert(furry_hal_crypto_mutex);
    furry_check(furry_mutex_acquire(furry_hal_crypto_mutex, FurryWaitForever) == FurryStatusOk);
    ...
    furry_hal_crypto_mode_init_done = false;
...
bool furry_hal_crypto_store_unload_key(uint8_t slot) {
    ...
    furry_assert(shci_state == SHCI_Success);
...
bool furry_hal_crypto_decrypt(const uint8_t* input, uint8_t* output, size_t size) {
    ...
    if(!furry_hal_crypto_mode_init_done) {
        ...
        furry_hal_crypto_mode_init_done = true;
    }
