...
static uint8_t furry_hal_gpio_get_pin_num(const GpioPin* gpio) {
    uint8_t pin_num = 0;
...
void furry_hal_gpio_init_simple(const GpioPin* gpio, const GpioMode mode) {
    furry_hal_gpio_init(gpio, mode, GpioPullNo, GpioSpeedLow);
}

void furry_hal_gpio_init(
    const GpioPin* gpio,
    const GpioMode mode,
    const GpioPull pull,
    const GpioSpeed speed) {
    // we cannot set alternate mode in this function
    furry_assert(mode != GpioModeAltFunctionPushPull);
    furry_assert(mode != GpioModeAltFunctionOpenDrain);

    furry_hal_gpio_init_ex(gpio, mode, pull, speed, GpioAltFnUnused);
}

void furry_hal_gpio_init_ex(
    const GpioPin* gpio,
    const GpioMode mode,
    const GpioPull pull,
    const GpioSpeed speed,
    ...
    if(mode >= GpioModeInterruptRise) {
        ...
        if(mode == GpioModeAltFunctionPushPull || mode == GpioModeAltFunctionOpenDrain) {
            ...
            if(furry_hal_gpio_get_pin_num(gpio) < 8) {
...
void furry_hal_gpio_add_int_callback(const GpioPin* gpio, GpioExtiCallback cb, void* ctx) {
    furry_assert(gpio);
    furry_assert(cb);

    FURRY_CRITICAL_ENTER();
    uint8_t pin_num = furry_hal_gpio_get_pin_num(gpio);
...
void furry_hal_gpio_enable_int_callback(const GpioPin* gpio) {
    furry_assert(gpio);
...
void furry_hal_gpio_disable_int_callback(const GpioPin* gpio) {
    furry_assert(gpio);
...
void furry_hal_gpio_remove_int_callback(const GpioPin* gpio) {
    furry_assert(gpio);
...
static void furry_hal_gpio_int_call(uint16_t pin_num) {
...
void EXTI9_5_IRQHandler(void) {
    if(LL_EXTI_IsActiveFlag_0_31(LL_EXTI_LINE_5)) {
        furry_hal_gpio_int_call(5);
        LL_EXTI_ClearFlag_0_31(LL_EXTI_LINE_5);
    }
    if(LL_EXTI_IsActiveFlag_0_31(LL_EXTI_LINE_6)) {
        furry_hal_gpio_int_call(6);
        LL_EXTI_ClearFlag_0_31(LL_EXTI_LINE_6);
    }
    if(LL_EXTI_IsActiveFlag_0_31(LL_EXTI_LINE_7)) {
        furry_hal_gpio_int_call(7);
        LL_EXTI_ClearFlag_0_31(LL_EXTI_LINE_7);
    }
    if(LL_EXTI_IsActiveFlag_0_31(LL_EXTI_LINE_8)) {
        furry_hal_gpio_int_call(8);
        LL_EXTI_ClearFlag_0_31(LL_EXTI_LINE_8);
    }
    if(LL_EXTI_IsActiveFlag_0_31(LL_EXTI_LINE_9)) {
        furry_hal_gpio_int_call(9);
...
void EXTI15_10_IRQHandler(void) {
    if(LL_EXTI_IsActiveFlag_0_31(LL_EXTI_LINE_10)) {
        furry_hal_gpio_int_call(10);
        LL_EXTI_ClearFlag_0_31(LL_EXTI_LINE_10);
    }
    if(LL_EXTI_IsActiveFlag_0_31(LL_EXTI_LINE_11)) {
        furry_hal_gpio_int_call(11);
        LL_EXTI_ClearFlag_0_31(LL_EXTI_LINE_11);
    }
    if(LL_EXTI_IsActiveFlag_0_31(LL_EXTI_LINE_12)) {
        furry_hal_gpio_int_call(12);
        LL_EXTI_ClearFlag_0_31(LL_EXTI_LINE_12);
    }
    if(LL_EXTI_IsActiveFlag_0_31(LL_EXTI_LINE_13)) {
        furry_hal_gpio_int_call(13);
        LL_EXTI_ClearFlag_0_31(LL_EXTI_LINE_13);
    }
    if(LL_EXTI_IsActiveFlag_0_31(LL_EXTI_LINE_14)) {
        furry_hal_gpio_int_call(14);
        LL_EXTI_ClearFlag_0_31(LL_EXTI_LINE_14);
    }
    if(LL_EXTI_IsActiveFlag_0_31(LL_EXTI_LINE_15)) {
        furry_hal_gpio_int_call(15);
