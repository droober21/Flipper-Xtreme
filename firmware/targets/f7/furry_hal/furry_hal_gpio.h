 ...
 */
static inline void furry_hal_gpio_write(const GpioPin* gpio, const bool state) {
 ...
 */
static inline void
    furry_hal_gpio_write_port_pin(GPIO_TypeDef* port, uint16_t pin, const bool state) {
    // writing to BSSR is an atomic operation
    if(state == true) {
        port->BSRR = pin;
    } else {
 ...
 * @return true / false
 */
static inline bool furry_hal_gpio_read(const GpioPin* gpio) {
 ...
 * @return true / false
 */
static inline bool furry_hal_gpio_read_port_pin(GPIO_TypeDef* port, uint16_t pin) {
