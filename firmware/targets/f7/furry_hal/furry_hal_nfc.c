...
void furry_hal_nfc_init() {
    ...
    if(ret == ERR_NONE) {
        ...
        FURRY_LOG_W(TAG, "Init Failed, RFAL returned: %d", ret);
    }
}
...
void furry_hal_nfc_deinit() {
    ...
    if(ret == ERR_NONE) {
        FURRY_LOG_I(TAG, "Deinit OK");
    } else {
        FURRY_LOG_W(TAG, "Deinit Failed, RFAL returned: %d", ret);
    }
...
bool furry_hal_nfc_detect(FurryHalNfcDevData* nfc_data, uint32_t timeout) {
    ...
    while(true) {
        ...
        if(state != state_old) {
            FURRY_LOG_T(TAG, "State change %d -> %d", state_old, state);
        }
        state_old = state;
    ...
    if(detected) {
        if(dev_list[0].type == RFAL_NFC_LISTEN_TYPE_NFCA) {
            ...
            furry_assert(dev_list[0].nfcidLen == RFAL_NFCF_NFCID2_LEN);
            memcpy(
...
bool furry_hal_nfc_activate_nfca(uint32_t timeout, uint32_t* cuid) {
    ...
    while(state != RFAL_NFC_STATE_ACTIVATED) {
        rfalNfcWorker();
        state = rfalNfcGetState();
        FURRY_LOG_T(TAG, "Current state %d", state);
        if(state == RFAL_NFC_STATE_POLL_ACTIVATION) {
            start = DWT->CYCCNT;
            continue;
        }
    ...
    if(cuid) {
        ...
        *cuid = (cuid_start[0] << 24) | (cuid_start[1] << 16) | (cuid_start[2] << 8) |
                (cuid_start[3]);
        FURRY_LOG_T(TAG, "Activated tag with cuid: %lX", *cuid);
    }
    return true;
}
...
bool furry_hal_nfc_listen_rx(FurryHalNfcTxRxContext* tx_rx, uint32_t timeout_ms) {
    ...
    // Wait for interrupts
    uint32_t start = furry_get_tick();
    bool data_received = false;
    while(true) {
        if(furry_hal_gpio_read(&gpio_nfc_irq_rfid_pull) == true) {
        ...
        if(furry_get_tick() - start > timeout_ms) {
            FURRY_LOG_T(TAG, "Interrupt waiting timeout");
            furry_delay_tick(1);
            break;
        }
    }
...
void furry_hal_nfc_listen_start(FurryHalNfcDevData* nfc_data) {
    ...
    furry_hal_gpio_init(&gpio_nfc_irq_rfid_pull, GpioModeInput, GpioPullDown, GpioSpeedVeryHigh);
    // Clear interrupts
    ...
    if(nfc_data->uid_len == 4) {
        pt_memory[12] = nfc_data->a_data.sak & ~FURRY_HAL_NFC_UID_INCOMPLETE;
    } else {
        pt_memory[12] = FURRY_HAL_NFC_UID_INCOMPLETE;
    }
    pt_memory[13] = nfc_data->a_data.sak & ~FURRY_HAL_NFC_UID_INCOMPLETE;
    pt_memory[14] = nfc_data->a_data.sak & ~FURRY_HAL_NFC_UID_INCOMPLETE;
...
void rfal_interrupt_callback_handler() {
    furry_event_flag_set(event, EVENT_FLAG_INTERRUPT);
}

void rfal_state_changed_callback(void* context) {
    UNUSED(context);
    furry_event_flag_set(event, EVENT_FLAG_STATE_CHANGED);
}

void furry_hal_nfc_stop() {
    if(event) {
        furry_event_flag_set(event, EVENT_FLAG_STOP);
    }
}
...
bool furry_hal_nfc_emulate_nfca(
    ...
    if(rfalListenStart(
           ...
           &buff_rx_len)) {
        rfalListenStop();
        FURRY_LOG_E(TAG, "Failed to start listen mode");
        return false;
    }
    while(true) {
        buff_rx_len = 0;
        buff_tx_len = 0;
        uint32_t flag = furry_event_flag_wait(event, EVENT_FLAG_ALL, FurryFlagWaitAny, timeout);
        if(flag == (unsigned)FurryFlagErrorTimeout || flag == EVENT_FLAG_STOP) {
            break;
        }
        bool data_received = false;
        buff_rx_len = 0;
        ...
        if(data_received) {
            ...
            if(nfca_emulation_handler(buff_rx, buff_rx_len, buff_tx, &buff_tx_len)) {
                if(rfalListenSleepStart(
                       ...
                       rfalConvBytesToBits(buff_rx_size),
                       &buff_rx_len)) {
                    FURRY_LOG_E(TAG, "Failed to enter sleep mode");
                    break;
                } else {
                    continue;
                }
            ...
            if(buff_tx_len) {
                ReturnCode ret = rfalTransceiveBitsBlockingTx(
                    ...
                    RFAL_FWT_NONE);
                if(ret) {
                    FURRY_LOG_E(TAG, "Tranceive failed with status %d", ret);
                    break;
                }
                continue;
            }
...
static bool furry_hal_nfc_transparent_tx_rx(FurryHalNfcTxRxContext* tx_rx, uint16_t timeout_ms) {
    furry_assert(tx_rx->nfca_signal);
    ...
    while(true) {
        ...
        if(timeout / furry_hal_cortex_instructions_per_microsecond() > timeout_ms * 1000) {
            FURRY_LOG_D(TAG, "Interrupt waiting timeout");
            break;
        }
    }
    if(irq) {
...
static uint16_t furry_hal_nfc_data_and_parity_to_bitstream(
    ...
    for(uint16_t i = 0; i < len; i++) {
        next_par_bit = FURRY_BIT(parity[i / 8], 7 - (i % 8));
        if(curr_bit_pos % 8 == 0) {
...
uint16_t furry_hal_nfc_bitstream_to_data_and_parity(
    ...
    while(bit_processed < in_buff_bits) {
        ...
        out_parity[curr_byte / 8] |= FURRY_BIT(in_buff[bit_processed / 8 + 1], bit_processed % 8)
                                     << (7 - curr_byte % 8);
        bit_processed += 9;
        curr_byte++;
    }
...
bool furry_hal_nfc_tx_rx(FurryHalNfcTxRxContext* tx_rx, uint16_t timeout_ms) {
    ...
    if(ret != ERR_NONE) {
        FURRY_LOG_E(TAG, "Failed to start data exchange");
        return false;
    }
    ...
    while(state != RFAL_NFC_STATE_DATAEXCHANGE_DONE) {
        ...
        if(ret == ERR_BUSY) {
            if(DWT->CYCCNT - start > timeout_ms * clocks_in_ms) {
                FURRY_LOG_D(TAG, "Timeout during data exchange");
                return false;
            }
            continue;
        } else {
...
bool furry_hal_nfc_tx_rx_full(FurryHalNfcTxRxContext* tx_rx) {
    ...
    while(tx_rx->rx_bits && tx_rx->rx_data[0] == 0xAF) {
        ...
        if(part_len_bytes > FURRY_HAL_NFC_DATA_BUFF_SIZE - tx_rx->rx_bits / 8) {
            FURRY_LOG_W(TAG, "Overrun rx buf");
            return false;
        }
        if(part_len_bytes == 0) {
            FURRY_LOG_W(TAG, "Empty 0xAF response");
            return false;
        }
...
FurryHalNfcReturn
    furry_hal_nfc_ll_set_mode(FurryHalNfcMode mode, FurryHalNfcBitrate txBR, FurryHalNfcBitrate rxBR) {
