...
void furry_hal_pwm_start(FurryHalPwmOutputId channel, uint32_t freq, uint8_t duty) {
    if(channel == FurryHalPwmOutputIdTim1PA7) {
        ...
        furry_hal_pwm_set_params(channel, freq, duty);

        LL_TIM_EnableCounter(TIM1);
    } else if(channel == FurryHalPwmOutputIdLptim2PA4) {
        furry_hal_gpio_init_ex(
            &gpio_ext_pa4,
            GpioModeAltFunctionPushPull,
            GpioPullNo,
...
void furry_hal_pwm_stop(FurryHalPwmOutputId channel) {
    if(channel == FurryHalPwmOutputIdTim1PA7) {
        ...
        furry_hal_gpio_init_simple(&gpio_ext_pa4, GpioModeAnalog);
        FURRY_CRITICAL_ENTER();
        LL_LPTIM_DeInit(LPTIM2);
        FURRY_CRITICAL_EXIT();
    }
