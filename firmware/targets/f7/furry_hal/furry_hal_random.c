...
#define TAG "FurryHalRandom"

static uint32_t furry_hal_random_read_rng() {
...
uint32_t furry_hal_random_get() {
    while(LL_HSEM_1StepLock(HSEM, CFG_HW_RNG_SEMID))
        ;
    LL_RNG_Enable(RNG);

    const uint32_t random_val = furry_hal_random_read_rng();
...
void furry_hal_random_fill_buf(uint8_t* buf, uint32_t len) {
    while(LL_HSEM_1StepLock(HSEM, CFG_HW_RNG_SEMID))
        ;
    LL_RNG_Enable(RNG);
...
int rand() {
    return (furry_hal_random_get() & RAND_MAX);
}
