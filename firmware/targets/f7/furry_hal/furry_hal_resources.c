...
static void furry_hal_resources_init_input_pins(GpioMode mode) {
...
void furry_hal_resources_init_early() {
    furry_hal_resources_init_input_pins(GpioModeInput);

    // SD Card stepdown control
    furry_hal_gpio_write(&gpio_periph_power, 1);
    furry_hal_gpio_init(&gpio_periph_power, GpioModeOutputOpenDrain, GpioPullNo, GpioSpeedLow);

    // Display pins
    furry_hal_gpio_write(&gpio_display_rst_n, 1);
    furry_hal_gpio_init_simple(&gpio_display_rst_n, GpioModeOutputPushPull);
    furry_hal_gpio_init_simple(&gpio_display_di, GpioModeOutputPushPull);
    ...
    // Hard reset USB
    furry_hal_gpio_write(&gpio_usb_dm, 1);
    furry_hal_gpio_write(&gpio_usb_dp, 1);
    furry_hal_gpio_init_simple(&gpio_usb_dm, GpioModeOutputOpenDrain);
    furry_hal_gpio_init_simple(&gpio_usb_dp, GpioModeOutputOpenDrain);
    furry_hal_gpio_write(&gpio_usb_dm, 0);
    furry_hal_gpio_write(&gpio_usb_dp, 0);
    furry_delay_us(5); // Device Driven disconnect: 2.5us + extra to compensate cables
    furry_hal_gpio_write(&gpio_usb_dm, 1);
    furry_hal_gpio_write(&gpio_usb_dp, 1);
    furry_hal_gpio_init_simple(&gpio_usb_dm, GpioModeAnalog);
    furry_hal_gpio_init_simple(&gpio_usb_dp, GpioModeAnalog);
    furry_hal_gpio_write(&gpio_usb_dm, 0);
    furry_hal_gpio_write(&gpio_usb_dp, 0);
    ...
    furry_hal_gpio_init(&gpio_ext_pa6, GpioModeAnalog, GpioPullNo, GpioSpeedLow);
    furry_hal_gpio_init(&gpio_ext_pa7, GpioModeAnalog, GpioPullNo, GpioSpeedLow);
}

void furry_hal_resources_deinit_early() {
    furry_hal_resources_init_input_pins(GpioModeAnalog);
}

void furry_hal_resources_init() {
    // Button pins
    furry_hal_resources_init_input_pins(GpioModeInterruptRiseFall);
    ...
    // Display pins
    furry_hal_gpio_init(&gpio_display_rst_n, GpioModeOutputPushPull, GpioPullNo, GpioSpeedLow);
    furry_hal_gpio_write(&gpio_display_rst_n, 0);

    furry_hal_gpio_init(&gpio_display_di, GpioModeOutputPushPull, GpioPullNo, GpioSpeedLow);
    furry_hal_gpio_write(&gpio_display_di, 0);

    // SD pins
    furry_hal_gpio_init(&gpio_sdcard_cd, GpioModeInput, GpioPullNo, GpioSpeedLow);
    furry_hal_gpio_write(&gpio_sdcard_cd, 0);

    furry_hal_gpio_init(&gpio_ibutton, GpioModeAnalog, GpioPullNo, GpioSpeedLow);

    furry_hal_gpio_init(&gpio_nfc_irq_rfid_pull, GpioModeInterruptRise, GpioPullNo, GpioSpeedLow);
...
int32_t furry_hal_resources_get_ext_pin_number(const GpioPin* gpio) {
