...
static bool furry_hal_usart_prev_enabled[2];

static void (*irq_cb[2])(uint8_t ev, uint8_t data, void* context);
static void* irq_ctx[2];

static void furry_hal_usart_init(uint32_t baud) {
...
static void furry_hal_lpuart_init(uint32_t baud) {
    furry_hal_gpio_init_ex(
        &gpio_ext_pc0,
        GpioModeAltFunctionPushPull,
        GpioPullUp,
    ...
    furry_hal_uart_set_br(FurryHalUartIdLPUART1, baud);
    LL_LPUART_DisableIT_ERROR(LPUART1);

    NVIC_SetPriority(LPUART1_IRQn, NVIC_EncodePriority(NVIC_GetPriorityGrouping(), 5, 0));
}

void furry_hal_uart_init(FurryHalUartId ch, uint32_t baud) {
    if(ch == FurryHalUartIdLPUART1)
        furry_hal_lpuart_init(baud);
    else if(ch == FurryHalUartIdUSART1)
        furry_hal_usart_init(baud);
}

void furry_hal_uart_set_br(FurryHalUartId ch, uint32_t baud) {
    if(ch == FurryHalUartIdUSART1) {
    ...
    } else if(ch == FurryHalUartIdLPUART1) {
...
void furry_hal_uart_deinit(FurryHalUartId ch) {
    furry_hal_uart_set_irq_cb(ch, NULL, NULL);
    if(ch == FurryHalUartIdUSART1) {
        LL_USART_Disable(USART1);
        furry_hal_gpio_init(&gpio_usart_tx, GpioModeAnalog, GpioPullNo, GpioSpeedLow);
        furry_hal_gpio_init(&gpio_usart_rx, GpioModeAnalog, GpioPullNo, GpioSpeedLow);
    } else if(ch == FurryHalUartIdLPUART1) {
        LL_LPUART_Disable(LPUART1);
...
void furry_hal_uart_suspend(FurryHalUartId channel) {
    if(channel == FurryHalUartIdLPUART1 && LL_LPUART_IsEnabled(LPUART1)) {
        LL_LPUART_Disable(LPUART1);
        furry_hal_usart_prev_enabled[channel] = true;
    } else if(channel == FurryHalUartIdUSART1 && LL_USART_IsEnabled(USART1)) {
        LL_USART_Disable(USART1);
        furry_hal_usart_prev_enabled[channel] = true;
    }
}
...
void furry_hal_uart_resume(FurryHalUartId channel) {
    if(!furry_hal_usart_prev_enabled[channel]) {
        return;
    } else if(channel == FurryHalUartIdLPUART1) {
        LL_LPUART_Enable(LPUART1);
    } else if(channel == FurryHalUartIdUSART1) {
        LL_USART_Enable(USART1);
    }

    furry_hal_usart_prev_enabled[channel] = false;
}

void furry_hal_uart_tx(FurryHalUartId ch, uint8_t* buffer, size_t buffer_size) {
    if(ch == FurryHalUartIdUSART1) {
...
void LPUART1_IRQHandler(void) {
    if(LL_LPUART_IsActiveFlag_RXNE_RXFNE(LPUART1)) {
        ...
        irq_cb[FurryHalUartIdLPUART1](UartIrqEventRXNE, data, irq_ctx[FurryHalUartIdLPUART1]);
...
void USART1_IRQHandler(void) {
    if(LL_USART_IsActiveFlag_RXNE_RXFNE(USART1)) {
        ...
        irq_cb[FurryHalUartIdUSART1](UartIrqEventRXNE, data, irq_ctx[FurryHalUartIdUSART1]);
