...
typedef struct {
    FurryHalUsbStateCallback callback;
    void* context;
} UsbApiEventDataStateCallback;

typedef struct {
    FurryHalUsbInterface* interface;
    void* context;
...
static int32_t furry_hal_usb_thread(void* context);
...
void furry_hal_usb_init(void) {
    ...
    usb.queue = furry_message_queue_alloc(1, sizeof(UsbApiEventMessage));
    usb.thread = furry_thread_alloc_ex("UsbDriver", 1024, furry_hal_usb_thread, NULL);
    furry_thread_mark_as_service(usb.thread);
    furry_thread_start(usb.thread);

    FURRY_LOG_I(TAG, "Init OK");
}

static void furry_hal_usb_send_message(UsbApiEventMessage* message) {
    furry_message_queue_put(usb.queue, message, FurryWaitForever);
    furry_thread_flags_set(furry_thread_get_id(usb.thread), UsbEventMessage);
...
bool furry_hal_usb_set_config(FurryHalUsbInterface* new_if, void* ctx) {
    ...
    furry_hal_usb_send_message(&msg);
...
static usbd_respond usb_descriptor_get(usbd_ctlreq* req, void** address, uint16_t* length) {
    ...
    case USB_DTYPE_DEVICE:
        furry_thread_flags_set(furry_thread_get_id(usb.thread), UsbEventRequest);
        if(usb.callback != NULL) {
            usb.callback(FurryHalUsbStateEventDescriptorRequest, usb.callback_context);
        }
...
static void reset_evt(usbd_device* dev, uint8_t event, uint8_t ep) {
    ...
    UNUSED(event);
    UNUSED(ep);
    furry_thread_flags_set(furry_thread_get_id(usb.thread), UsbEventReset);
    if(usb.callback != NULL) {
        usb.callback(FurryHalUsbStateEventReset, usb.callback_context);
    }
}
...
static void susp_evt(usbd_device* dev, uint8_t event, uint8_t ep) {
    ...
    if(usb.callback != NULL) {
        usb.callback(FurryHalUsbStateEventSuspend, usb.callback_context);
    }
}
...
static void wkup_evt(usbd_device* dev, uint8_t event, uint8_t ep) {
    ...
    if(usb.callback != NULL) {
        usb.callback(FurryHalUsbStateEventWakeup, usb.callback_context);
    }
}

static void usb_process_mode_start(FurryHalUsbInterface* interface, void* context) {
    ...
    if(interface != NULL) {
        ...
        FURRY_LOG_I(TAG, "USB Mode change done");
        usb.enabled = true;
    }
}

static void usb_process_mode_change(FurryHalUsbInterface* interface, void* context) {
    if((interface != usb.interface) || (context != usb.interface_context)) {
        if(usb.enabled) {
            ...
            usb.enabled = false;
            furry_delay_ms(USB_RECONNECT_DELAY);
        }
...
static bool usb_process_set_config(FurryHalUsbInterface* interface, void* context) {
    if(usb.mode_lock) {
        return false;
    } else {
...
static void usb_process_message(UsbApiEventMessage* message) {
    ...
    case UsbApiEventTypeLock:
        FURRY_LOG_I(TAG, "Mode lock");
        usb.mode_lock = true;
        break;
    case UsbApiEventTypeUnlock:
        FURRY_LOG_I(TAG, "Mode unlock");
        usb.mode_lock = false;
        break;
...
static int32_t furry_hal_usb_thread(void* context) {
    ...
    if(furry_message_queue_get_count(usb.queue) > 0) {
        furry_thread_flags_set(furry_thread_get_id(usb.thread), UsbEventMessage);
    }

    while(true) {
        uint32_t flags = furry_thread_flags_wait(USB_SRV_ALL_EVENTS, FurryFlagWaitAny, 500);

        {
            UsbApiEventMessage message;
            if(furry_message_queue_get(usb.queue, &message, 0) == FurryStatusOk) {
                usb_process_message(&message);
            }
        }

        if((flags & FurryFlagError) == 0) {
