...
static void cdc_init(usbd_device* dev, FurryHalUsbInterface* intf, void* ctx);
...
FurryHalUsbInterface usb_cdc_dual = {
    .init = cdc_init,
    .deinit = cdc_deinit,
    .wakeup = cdc_on_wakeup,
    .suspend = cdc_on_suspend,
...
static void cdc_init(usbd_device* dev, FurryHalUsbInterface* intf, void* ctx) {
    ...
    char* name = (char*)furry_hal_version_get_device_name_ptr();
    ...
    name = (char*)furry_hal_version_get_name_ptr();
...
void furry_hal_cdc_set_callbacks(uint8_t if_num, CdcCallbacks* cb, void* context) {
    furry_assert(if_num < IF_NUM_MAX);
...
struct usb_cdc_line_coding* furry_hal_cdc_get_port_settings(uint8_t if_num) {
    furry_assert(if_num < IF_NUM_MAX);
    return &cdc_config[if_num];
}

uint8_t furry_hal_cdc_get_ctrl_line_state(uint8_t if_num) {
    furry_assert(if_num < IF_NUM_MAX);
    return cdc_ctrl_line_state[if_num];
}

void furry_hal_cdc_send(uint8_t if_num, uint8_t* buf, uint16_t len) {
    if(if_num == 0)
...
int32_t furry_hal_cdc_receive(uint8_t if_num, uint8_t* buf, uint16_t max_len) {
    int32_t len = 0;
    if(if_num == 0)
