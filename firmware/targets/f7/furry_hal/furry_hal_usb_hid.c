...
static FurrySemaphore* hid_semaphore = NULL;
...
bool furry_hal_hid_is_connected() {
    return hid_connected;
}

uint8_t furry_hal_hid_get_led_state() {
    return led_state;
}

void furry_hal_hid_set_state_callback(HidStateCallback cb, void* ctx) {
    if(callback != NULL) {
...
bool furry_hal_hid_kb_press(uint16_t button) {
...
bool furry_hal_hid_kb_release(uint16_t button) {
    for(uint8_t key_nb = 0; key_nb < HID_KB_MAX_KEYS; key_nb++) {
...
bool furry_hal_hid_kb_release_all() {
    for(uint8_t key_nb = 0; key_nb < HID_KB_MAX_KEYS; key_nb++) {
        hid_report.keyboard.boot.btn[key_nb] = 0;
    }
...
bool furry_hal_hid_mouse_move(int8_t dx, int8_t dy) {
...
bool furry_hal_hid_mouse_press(uint8_t button) {
    hid_report.mouse.btn |= button;
    return hid_send_report(ReportIdMouse);
}

bool furry_hal_hid_mouse_release(uint8_t button) {
    hid_report.mouse.btn &= ~button;
    return hid_send_report(ReportIdMouse);
}

bool furry_hal_hid_mouse_scroll(int8_t delta) {
...
bool furry_hal_hid_consumer_key_press(uint16_t button) {
...
bool furry_hal_hid_consumer_key_release(uint16_t button) {
    for(uint8_t key_nb = 0; key_nb < HID_CONSUMER_MAX_KEYS; key_nb++) {
...
static void hid_init(usbd_device* dev, FurryHalUsbInterface* intf, void* ctx) {
    UNUSED(intf);
    FurryHalUsbHidConfig* cfg = (FurryHalUsbHidConfig*)ctx;
    if(hid_semaphore == NULL) hid_semaphore = furry_semaphore_alloc(1, 1);
    usb_dev = dev;
...
static void hid_on_suspend(usbd_device* dev) {
    ...
    if(hid_connected) {
        hid_connected = false;
        furry_semaphore_release(hid_semaphore);
        if(callback != NULL) {
...
static bool hid_send_report(uint8_t report_id) {
    ...
    furry_check(furry_semaphore_acquire(hid_semaphore, FurryWaitForever) == FurryStatusOk);
