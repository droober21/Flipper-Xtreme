...
typedef void FurryStreamBuffer;
 ...
 * Setting timeout to FurryWaitForever will cause the task to wait indefinitely.
 ...
 * 
 * @param stream_buffer The stream buffer instance.
 * @return FurryStatusOk if the stream buffer is reset. 
 * @return FurryStatusError if there was a task blocked waiting to send to or read 
