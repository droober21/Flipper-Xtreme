...
bool timerIsExpired(uint32_t timer) {
    ...
    uDiff = (timer - furry_get_tick()); /* Calculate the diff between the timers */
...
void timerStopwatchStart(void) {
    timerStopwatchTick = furry_get_tick();
}

/*******************************************************************************/
uint32_t timerStopwatchMeasure(void) {
    return (uint32_t)(furry_get_tick() - timerStopwatchTick);
}
