...
uint16_t bq27220_read_word(FurryHalI2cBusHandle* handle, uint8_t address) {
    uint16_t buf = 0;
...
bool bq27220_control(FurryHalI2cBusHandle* handle, uint16_t control) {
    bool ret = furry_hal_i2c_write_mem(
...
bool bq27220_set_parameter_u16(FurryHalI2cBusHandle* handle, uint16_t address, uint16_t value) {
    bool ret;
    uint8_t buffer[4];
...
bool bq27220_init(FurryHalI2cBusHandle* handle, const ParamCEDV* cedv) {
    ...
    if(cedv->design_cap == design_cap) {
        FURRY_LOG_I(TAG, "Skip battery profile update");
        return true;
    }
    FURRY_LOG_I(TAG, "Start updating battery profile");
    OperationStatus status = {0};
    if(!bq27220_control(handle, Control_ENTER_CFG_UPDATE)) {
        FURRY_LOG_E(TAG, "Can't configure update");
        return false;
    };
    ...
    if(cedv->design_cap == design_cap) {
        FURRY_LOG_I(TAG, "Battery profile update success");
        return true;
    } else {
        FURRY_LOG_E(TAG, "Battery profile update failed");
        return false;
    }
}
