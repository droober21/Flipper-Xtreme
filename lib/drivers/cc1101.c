...
CC1101Status cc1101_strobe(FurryHalSpiBusHandle* handle, uint8_t strobe) {
    ...
    while(furry_hal_gpio_read(handle->miso))
        ;
    furry_hal_spi_bus_trx(handle, tx, (uint8_t*)rx, 1, CC1101_TIMEOUT);
...
CC1101Status cc1101_write_reg(FurryHalSpiBusHandle* handle, uint8_t reg, uint8_t data) {
    ...
    while(furry_hal_gpio_read(handle->miso))
        ;
    furry_hal_spi_bus_trx(handle, tx, (uint8_t*)rx, 2, CC1101_TIMEOUT);
...
void cc1101_set_pa_table(FurryHalSpiBusHandle* handle, const uint8_t value[8]) {
    ...
    while(furry_hal_gpio_read(handle->miso))
        ;
    furry_hal_spi_bus_trx(handle, tx, (uint8_t*)rx, sizeof(rx), CC1101_TIMEOUT);
...
uint8_t cc1101_write_fifo(FurryHalSpiBusHandle* handle, const uint8_t* data, uint8_t size) {
    ...
    furry_hal_spi_bus_trx(handle, buff_tx, (uint8_t*)buff_rx, size + 1, CC1101_TIMEOUT);
...
uint8_t cc1101_read_fifo(FurryHalSpiBusHandle* handle, uint8_t* data, uint8_t* size) {
    ...
    furry_hal_spi_bus_trx(handle, buff_trx, buff_trx, 2, CC1101_TIMEOUT);
    ...
    furry_hal_spi_bus_trx(handle, NULL, data, *size, CC1101_TIMEOUT);
