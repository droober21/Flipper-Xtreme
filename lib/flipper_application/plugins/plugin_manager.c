...
PluginManager* plugin_manager_alloc(
    ...
    manager->storage = furry_record_open(RECORD_STORAGE);
...
PluginManagerError plugin_manager_load_single(PluginManager* manager, const char* path) {
    ...
    do {
        ...
        if(preload_res != FlipperApplicationPreloadStatusSuccess) {
            FURRY_LOG_E(TAG, "Failed to preload %s", path);
            error = PluginManagerErrorLoaderError;
            break;
        }

        if(!flipper_application_is_plugin(lib)) {
            FURRY_LOG_E(TAG, "Not a plugin %s", path);
            error = PluginManagerErrorLoaderError;
            break;
        }
        ...
        if(load_status != FlipperApplicationLoadStatusSuccess) {
            FURRY_LOG_E(TAG, "Failed to load module_demo_plugin1.fal");
            break;
        }
...
PluginManagerError plugin_manager_load_all(PluginManager* manager, const char* path) {
    ...
    do {
        if(!storage_dir_open(directory, path)) {
            FURRY_LOG_E(TAG, "Failed to open directory %s", path);
            break;
        }
        while(true) {
            ...
            furry_string_set(file_name, file_name_buffer);
            if(!furry_string_end_with_str(file_name, ".fal")) {
                continue;
            }

            path_concat(path, file_name_buffer, file_name);
            FURRY_LOG_D(TAG, "Loading %s", furry_string_get_cstr(file_name));
            PluginManagerError error =
                plugin_manager_load_single(manager, furry_string_get_cstr(file_name));

            if(error != PluginManagerErrorNone) {
                FURRY_LOG_E(TAG, "Failed to load %s", furry_string_get_cstr(file_name));
                break;
            }
        }
    } while(false);
