...
static bool flipper_format_stream_read_valid_key(Stream* stream, FurryString* key) {
        ...
        for(size_t i = 0; i < was_read; i++) {
            ...
            if(data == flipper_format_eoln) {
                ...
                if(accumulate) {
                    ...
                    furry_string_push_back(key, data);
                }
            }
        }
...
bool flipper_format_stream_seek_to_key(Stream* stream, const char* key, bool strict_mode) {
    bool found = false;
    FurryString* read_key;

    read_key = furry_string_alloc();

    while(!stream_eof(stream)) {
        if(flipper_format_stream_read_valid_key(stream, read_key)) {
            if(furry_string_cmp_str(read_key, key) == 0) {
    ...
    furry_string_free(read_key);

    return found;
}

static bool flipper_format_stream_read_value(Stream* stream, FurryString* value, bool* last) {
        ...
        for(uint16_t i = 0; i < was_read; i++) {
            ...
            if(state == LeadingSpace) {
                if(flipper_format_stream_is_space(data)) {
                    ...
                    state = ReadValue;
                    furry_string_push_back(value, data);
                }
...
static bool flipper_format_stream_read_line(Stream* stream, FurryString* str_result) {
    ...
    do {
        ...
        for(size_t i = 0; i < was_read; i++) {
            ...
            if(data == flipper_format_eoln) {
                ...
                furry_string_push_back(str_result, data);
            }
        }
        ...
        do {
            ...
            for(uint16_t i = 0; i < write_data->data_size; i++) {
                ...
                case FlipperStreamValueBool: {
                    ...
                    furry_string_printf(value, data[i] ? "true" : "false");
                }; break;
                default:
                    furry_crash("Unknown FF type");
                }

                if(((size_t)i + 1) < write_data->data_size) {
                    furry_string_cat(value, " ");
                }

                if(!flipper_format_stream_write(
                       stream, furry_string_get_cstr(value), furry_string_size(value))) {
                    cycle_error = true;
                    break;
                }
            }
...
bool flipper_format_stream_read_value_line(
    ...
    do {
        ...
        if(type == FlipperStreamValueStr) {
            FurryString* data = (FurryString*)_data;
            ...
            for(size_t i = 0; i < data_size; i++) {
                ...
                if(result) {
                    ...
                    case FlipperStreamValueHex: {
                        uint8_t* data = _data;
                        if(furry_string_size(value) >= 2) {
                            // sscanf "%02X" does not work here
                            if(hex_char_to_uint8(
                                   furry_string_get_char(value, 0),
                                   furry_string_get_char(value, 1),
                                   &data[i])) {
                                scan_values = 1;
                            }
                        }
                    ...
                    case FlipperStreamValueFloat: {
                        ...
                        // scan_values = sscanf(furry_string_get_cstr(value), "%f", &data[i]);
                        char* end_char;
                        data[i] = strtof(furry_string_get_cstr(value), &end_char);
                        if(*end_char == 0) {
                            // most likely ok
                            scan_values = 1;
                        }
                    ...
                    case FlipperStreamValueInt32: {
                        int32_t* data = _data;
                        scan_values = sscanf(furry_string_get_cstr(value), "%" PRIi32, &data[i]);
                    }; break;
                    case FlipperStreamValueUint32: {
                        uint32_t* data = _data;
                        scan_values = sscanf(furry_string_get_cstr(value), "%" PRIu32, &data[i]);
                    }; break;
                    case FlipperStreamValueHexUint64: {
                        uint64_t* data = _data;
                        if(furry_string_size(value) >= 16) {
                            if(hex_chars_to_uint64(furry_string_get_cstr(value), &data[i])) {
                                scan_values = 1;
                            }
                        }
                    }; break;
                    case FlipperStreamValueBool: {
                        bool* data = _data;
                        data[i] = !furry_string_cmpi(value, "true");
                        scan_values = 1;
                    }; break;
                    default:
                        furry_crash("Unknown FF type");
