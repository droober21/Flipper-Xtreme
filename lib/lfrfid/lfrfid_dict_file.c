...
bool lfrfid_dict_file_save(ProtocolDict* dict, ProtocolId protocol, const char* filename) {
    furry_check(protocol != PROTOCOL_NO);
...
ProtocolId lfrfid_dict_file_load(ProtocolDict* dict, const char* filename) {
    ...
    FurryString* str_result;
    str_result = furry_string_alloc();
    ...
    do {
        ...
        if(furry_string_cmp_str(str_result, LFRFID_DICT_FILETYPE) != 0) break;
        if(version != 1) break;

        // type
        if(!flipper_format_read_string(file, "Key type", str_result)) break;
        ProtocolId protocol;
        protocol = protocol_dict_get_protocol_by_name(dict, furry_string_get_cstr(str_result));

        if(protocol == PROTOCOL_NO) {
            protocol = lfrfid_dict_protocol_fallback(dict, furry_string_get_cstr(str_result), file);
