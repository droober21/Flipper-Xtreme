...
LFRFIDWorker* lfrfid_worker_alloc(ProtocolDict* dict) {
    ...
    worker->thread = furry_thread_alloc_ex("LfrfidWorker", 2048, lfrfid_worker_thread, worker);
...
void lfrfid_worker_read_start(
    ...
    void* context) {
    furry_assert(worker->mode_index == LFRFIDWorkerIdle);
    worker->read_type = type;
    worker->read_cb = callback;
    worker->cb_ctx = context;
    furry_thread_flags_set(furry_thread_get_id(worker->thread), LFRFIDEventRead);
}
...
void lfrfid_worker_write_start(
    ...
    worker->cb_ctx = context;
    furry_thread_flags_set(furry_thread_get_id(worker->thread), LFRFIDEventWrite);
}

void lfrfid_worker_emulate_start(LFRFIDWorker* worker, LFRFIDProtocol protocol) {
    furry_assert(worker->mode_index == LFRFIDWorkerIdle);
    worker->protocol = protocol;
    furry_thread_flags_set(furry_thread_get_id(worker->thread), LFRFIDEventEmulate);
}
...
void lfrfid_worker_read_raw_start(
    ...
    furry_thread_flags_set(furry_thread_get_id(worker->thread), LFRFIDEventReadRaw);
}
...
void lfrfid_worker_emulate_raw_start(
    ...
    worker->cb_ctx = context;
    furry_thread_flags_set(furry_thread_get_id(worker->thread), LFRFIDEventEmulateRaw);
}

void lfrfid_worker_stop(LFRFIDWorker* worker) {
    furry_thread_flags_set(furry_thread_get_id(worker->thread), LFRFIDEventStopMode);
}
...
void lfrfid_worker_stop_thread(LFRFIDWorker* worker) {
    furry_assert(worker->mode_index == LFRFIDWorkerIdle);
    furry_thread_flags_set(furry_thread_get_id(worker->thread), LFRFIDEventStopThread);
...
static int32_t lfrfid_worker_thread(void* thread_context) {
    ...
    while(true) {
        uint32_t flags = furry_thread_flags_wait(LFRFIDEventAll, FurryFlagWaitAny, FurryWaitForever);
