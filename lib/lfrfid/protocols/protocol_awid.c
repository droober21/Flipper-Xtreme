...
void protocol_awid_render_data(ProtocolAwid* protocol, FurryString* result) {
    // Index map
    // 0           10         20        30          40        50        60
    // |           |          |         |           |         |         |
    ...
    if(format_length == 26) {
        ...
        furry_string_cat_printf(result, "Facility: %d\r\n", facility);
        furry_string_cat_printf(result, "Card: %d", card_id);
    } else {
        // print 66 bits as hex
        furry_string_cat_printf(result, "Data: ");
        for(size_t i = 0; i < AWID_DECODED_DATA_SIZE; i++) {
            furry_string_cat_printf(result, "%02X", decoded_data[i]);
        }
    }
};

void protocol_awid_render_brief_data(ProtocolAwid* protocol, FurryString* result) {
    uint8_t* decoded_data = protocol->data;
    uint8_t format_length = decoded_data[0];
    ...
    if(format_length == 26) {
        ...
        bit_lib_copy_bits((uint8_t*)&card_id, 8, 8, decoded_data, 17);
        bit_lib_copy_bits((uint8_t*)&card_id, 0, 8, decoded_data, 25);
        furry_string_cat_printf(result, "ID: %03u,%05u", facility, card_id);
    } else {
