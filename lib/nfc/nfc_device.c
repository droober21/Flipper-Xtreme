...
NfcDevice* nfc_device_alloc() {
    ...
    nfc_dev->storage = furry_record_open(RECORD_STORAGE);
    nfc_dev->dialogs = furry_record_open(RECORD_DIALOGS);
    nfc_dev->load_path = furry_string_alloc();
    nfc_dev->dev_data.parsed_data = furry_string_alloc();
    nfc_dev->folder = furry_string_alloc();
...
void nfc_device_free(NfcDevice* nfc_dev) {
    furry_assert(nfc_dev);
    nfc_device_clear(nfc_dev);
    furry_record_close(RECORD_STORAGE);
    furry_record_close(RECORD_DIALOGS);
    furry_string_free(nfc_dev->load_path);
    furry_string_free(nfc_dev->dev_data.parsed_data);
    furry_string_free(nfc_dev->folder);
    free(nfc_dev);
}

static void nfc_device_prepare_format_string(NfcDevice* dev, FurryString* format_string) {
    if(dev->format == NfcDeviceSaveFormatUid) {
        furry_string_set(format_string, "UID");
    } else if(dev->format == NfcDeviceSaveFormatBankCard) {
        furry_string_set(format_string, "Bank card");
    } else if(dev->format == NfcDeviceSaveFormatMifareUl) {
        furry_string_set(format_string, nfc_mf_ul_type(dev->dev_data.mf_ul_data.type, true));
    } else if(dev->format == NfcDeviceSaveFormatMifareClassic) {
        furry_string_set(format_string, "Mifare Classic");
    } else if(dev->format == NfcDeviceSaveFormatMifareDesfire) {
        furry_string_set(format_string, "Mifare DESFire");
    } else if(dev->format == NfcDeviceSaveFormatNfcV) {
        furry_string_set(format_string, "ISO15693");
    } else {
        furry_string_set(format_string, "Unknown");
    }
}

static bool nfc_device_parse_format_string(NfcDevice* dev, FurryString* format_string) {
    if(furry_string_start_with_str(format_string, "UID")) {
        dev->format = NfcDeviceSaveFormatUid;
        dev->dev_data.protocol = NfcDeviceProtocolUnknown;
        return true;
    }
    if(furry_string_start_with_str(format_string, "Bank card")) {
    ...
    for(MfUltralightType type = MfUltralightTypeUnknown; type < MfUltralightTypeNum; type++) {
        if(furry_string_equal(format_string, nfc_mf_ul_type(type, true))) {
...
static bool nfc_device_save_mifare_ul_data(FlipperFormat* file, NfcDevice* dev) {
    ...
    do {
        ...
        for(uint8_t i = 0; i < 3; i++) {
            furry_string_printf(temp_str, "Counter %d", i);
            if(!flipper_format_write_uint32(
                   file, furry_string_get_cstr(temp_str), &data->counter[i], 1)) {
                counters_saved = false;
                break;
            }
            furry_string_printf(temp_str, "Tearing %d", i);
            if(!flipper_format_write_hex(
                   file, furry_string_get_cstr(temp_str), &data->tearing[i], 1)) {
                counters_saved = false;
                break;
            }
        }
        ...
        for(uint16_t i = 0; i < data->data_size; i += 4) {
            furry_string_printf(temp_str, "Page %d", i / 4);
            if(!flipper_format_write_hex(file, furry_string_get_cstr(temp_str), &data->data[i], 4)) {
                pages_saved = false;
                break;
            }
        }
...
bool nfc_device_load_mifare_ul_data(FlipperFormat* file, NfcDevice* dev) {
    ...
    do {
        ...
        for(uint16_t i = 0; i < pages_total; i++) {
            furry_string_printf(temp_str, "Page %d", i);
            if(!flipper_format_read_hex(
                   file, furry_string_get_cstr(temp_str), &data->data[i * 4], 4)) {
                pages_parsed = false;
                break;
            }
        }
...
static bool nfc_device_save_mifare_df_key_settings(
    ...
    do {
        furry_string_printf(key, "%s Change Key ID", prefix);
        if(!flipper_format_write_hex(file, furry_string_get_cstr(key), &ks->change_key_id, 1))
            break;
        furry_string_printf(key, "%s Config Changeable", prefix);
        if(!flipper_format_write_bool(file, furry_string_get_cstr(key), &ks->config_changeable, 1))
            break;
        furry_string_printf(key, "%s Free Create Delete", prefix);
        if(!flipper_format_write_bool(file, furry_string_get_cstr(key), &ks->free_create_delete, 1))
            break;
        furry_string_printf(key, "%s Free Directory List", prefix);
        if(!flipper_format_write_bool(file, furry_string_get_cstr(key), &ks->free_directory_list, 1))
            break;
        furry_string_printf(key, "%s Key Changeable", prefix);
        if(!flipper_format_write_bool(
               file, furry_string_get_cstr(key), &ks->master_key_changeable, 1))
            break;
        if(ks->flags) {
            furry_string_printf(key, "%s Flags", prefix);
            if(!flipper_format_write_hex(file, furry_string_get_cstr(key), &ks->flags, 1)) break;
        }
        furry_string_printf(key, "%s Max Keys", prefix);
        if(!flipper_format_write_hex(file, furry_string_get_cstr(key), &ks->max_keys, 1)) break;
        for(MifareDesfireKeyVersion* kv = ks->key_version_head; kv; kv = kv->next) {
            furry_string_printf(key, "%s Key %d Version", prefix, kv->id);
            if(!flipper_format_write_hex(file, furry_string_get_cstr(key), &kv->version, 1)) break;
        }
        saved = true;
    } while(false);
...
bool nfc_device_load_mifare_df_key_settings(
    ...
    do {
        furry_string_printf(key, "%s Change Key ID", prefix);
        if(!flipper_format_read_hex(file, furry_string_get_cstr(key), &ks->change_key_id, 1)) break;
        furry_string_printf(key, "%s Config Changeable", prefix);
        if(!flipper_format_read_bool(file, furry_string_get_cstr(key), &ks->config_changeable, 1))
            break;
        furry_string_printf(key, "%s Free Create Delete", prefix);
        if(!flipper_format_read_bool(file, furry_string_get_cstr(key), &ks->free_create_delete, 1))
            break;
        furry_string_printf(key, "%s Free Directory List", prefix);
        if(!flipper_format_read_bool(file, furry_string_get_cstr(key), &ks->free_directory_list, 1))
            break;
        furry_string_printf(key, "%s Key Changeable", prefix);
        if(!flipper_format_read_bool(
            ...
            break;
        furry_string_printf(key, "%s Flags", prefix);
        if(flipper_format_key_exist(file, furry_string_get_cstr(key))) {
            if(!flipper_format_read_hex(file, furry_string_get_cstr(key), &ks->flags, 1)) break;
        }
        furry_string_printf(key, "%s Max Keys", prefix);
        if(!flipper_format_read_hex(file, furry_string_get_cstr(key), &ks->max_keys, 1)) break;
        ks->flags |= ks->max_keys >> 4;
        ks->max_keys &= 0xF;
        MifareDesfireKeyVersion** kv_head = &ks->key_version_head;
        for(int key_id = 0; key_id < ks->max_keys; key_id++) {
            furry_string_printf(key, "%s Key %d Version", prefix, key_id);
            uint8_t version;
            if(flipper_format_read_hex(file, furry_string_get_cstr(key), &version, 1)) {
...
static bool nfc_device_save_mifare_df_app(FlipperFormat* file, MifareDesfireApplication* app) {
    bool saved = false;
    FurryString *prefix, *key;
    prefix =
        furry_string_alloc_printf("Application %02x%02x%02x", app->id[0], app->id[1], app->id[2]);
    key = furry_string_alloc();
    uint8_t* tmp = NULL;
    ...
    do {
        if(app->key_settings) {
            if(!nfc_device_save_mifare_df_key_settings(
                   file, app->key_settings, furry_string_get_cstr(prefix)))
                break;
        }
        ...
        furry_string_printf(key, "%s File IDs", furry_string_get_cstr(prefix));
        if(!flipper_format_write_hex(file, furry_string_get_cstr(key), tmp, n_files)) break;
        bool saved_files = true;
        for(MifareDesfireFile* f = app->file_head; f; f = f->next) {
            saved_files = false;
            furry_string_printf(key, "%s File %d Type", furry_string_get_cstr(prefix), f->id);
            if(!flipper_format_write_hex(file, furry_string_get_cstr(key), &f->type, 1)) break;
            furry_string_printf(
                key, "%s File %d Communication Settings", furry_string_get_cstr(prefix), f->id);
            if(!flipper_format_write_hex(file, furry_string_get_cstr(key), &f->comm, 1)) break;
            furry_string_printf(
                key, "%s File %d Access Rights", furry_string_get_cstr(prefix), f->id);
            if(!flipper_format_write_hex(
                   file, furry_string_get_cstr(key), (uint8_t*)&f->access_rights, 2))
                break;
            uint16_t size = 0;
            if(f->type == MifareDesfireFileTypeStandard ||
               f->type == MifareDesfireFileTypeBackup) {
                size = f->settings.data.size;
                furry_string_printf(key, "%s File %d Size", furry_string_get_cstr(prefix), f->id);
                if(!flipper_format_write_uint32(
                       file, furry_string_get_cstr(key), &f->settings.data.size, 1))
                    break;
            } else if(f->type == MifareDesfireFileTypeValue) {
                furry_string_printf(
                    key, "%s File %d Hi Limit", furry_string_get_cstr(prefix), f->id);
                if(!flipper_format_write_uint32(
                       file, furry_string_get_cstr(key), &f->settings.value.hi_limit, 1))
                    break;
                furry_string_printf(
                    key, "%s File %d Lo Limit", furry_string_get_cstr(prefix), f->id);
                if(!flipper_format_write_uint32(
                       file, furry_string_get_cstr(key), &f->settings.value.lo_limit, 1))
                    break;
                furry_string_printf(
                    key, "%s File %d Limited Credit Value", furry_string_get_cstr(prefix), f->id);
                if(!flipper_format_write_uint32(
                       file, furry_string_get_cstr(key), &f->settings.value.limited_credit_value, 1))
                    break;
                furry_string_printf(
                    key, "%s File %d Limited Credit Enabled", furry_string_get_cstr(prefix), f->id);
                if(!flipper_format_write_bool(
                       file,
                       furry_string_get_cstr(key),
                ...
                furry_string_printf(key, "%s File %d Size", furry_string_get_cstr(prefix), f->id);
                if(!flipper_format_write_uint32(
                       file, furry_string_get_cstr(key), &f->settings.record.size, 1))
                    break;
                furry_string_printf(key, "%s File %d Max", furry_string_get_cstr(prefix), f->id);
                if(!flipper_format_write_uint32(
                       file, furry_string_get_cstr(key), &f->settings.record.max, 1))
                    break;
                furry_string_printf(key, "%s File %d Cur", furry_string_get_cstr(prefix), f->id);
                if(!flipper_format_write_uint32(
                       file, furry_string_get_cstr(key), &f->settings.record.cur, 1))
                    break;
                size = f->settings.record.size * f->settings.record.cur;
            }
            if(f->contents) {
                furry_string_printf(key, "%s File %d", furry_string_get_cstr(prefix), f->id);
                if(!flipper_format_write_hex(file, furry_string_get_cstr(key), f->contents, size))
                    break;
            }
            saved_files = true;
        }
...
bool nfc_device_load_mifare_df_app(FlipperFormat* file, MifareDesfireApplication* app) {
    ...
    do {
        ...
        furry_string_printf(key, "%s File IDs", furry_string_get_cstr(prefix));
        uint32_t n_files;
        if(!flipper_format_get_value_count(file, furry_string_get_cstr(key), &n_files)) break;
        tmp = malloc(n_files);
        if(!flipper_format_read_hex(file, furry_string_get_cstr(key), tmp, n_files)) break;
        ...
        for(uint32_t i = 0; i < n_files; i++) {
            ...
            f->id = tmp[i];
            furry_string_printf(key, "%s File %d Type", furry_string_get_cstr(prefix), f->id);
            if(!flipper_format_read_hex(file, furry_string_get_cstr(key), &f->type, 1)) break;
            furry_string_printf(
                key, "%s File %d Communication Settings", furry_string_get_cstr(prefix), f->id);
            if(!flipper_format_read_hex(file, furry_string_get_cstr(key), &f->comm, 1)) break;
            furry_string_printf(
                key, "%s File %d Access Rights", furry_string_get_cstr(prefix), f->id);
            if(!flipper_format_read_hex(
                   file, furry_string_get_cstr(key), (uint8_t*)&f->access_rights, 2))
            ...
            if(flipper_format_key_exist(file, furry_string_get_cstr(key))) {
                uint32_t size;
                if(!flipper_format_get_value_count(file, furry_string_get_cstr(key), &size)) break;
                f->contents = malloc(size);
                if(!flipper_format_read_hex(file, furry_string_get_cstr(key), f->contents, size))
                    break;
            }
            *file_head = f;
            file_head = &f->next;
    ...
    do {
        ...
        if(n_apps) {
            ...
            if(!parsed_apps) {
                ...
                FURRY_LOG_W(TAG, "Non-parsed apps found!");
            }
        }
        parsed = true;
    } while(false);
...
bool nfc_device_load_bank_card_data(FlipperFormat* file, NfcDevice* dev) {
    ...
    do {
        ...
        strlcpy(data->name, furry_string_get_cstr(temp_str), sizeof(data->name));
...
static void nfc_device_write_mifare_classic_block(
    FurryString* block_str,
    MfClassicData* data,
    uint8_t block_num) {
    furry_string_reset(block_str);
    ...
    if(is_sec_trailer) {
        ...
        for(size_t i = 0; i < sizeof(sec_tr->key_a); i++) {
            if(mf_classic_is_key_found(data, sector_num, MfClassicKeyA)) {
                furry_string_cat_printf(block_str, "%02X ", sec_tr->key_a[i]);
            } else {
                furry_string_cat_printf(block_str, "?? ");
            }
        }
        // Write Access bytes
        for(size_t i = 0; i < MF_CLASSIC_ACCESS_BYTES_SIZE; i++) {
            if(mf_classic_is_block_read(data, block_num)) {
                furry_string_cat_printf(block_str, "%02X ", sec_tr->access_bits[i]);
            } else {
                furry_string_cat_printf(block_str, "?? ");
            }
        }
        ...
        for(size_t i = 0; i < sizeof(sec_tr->key_b); i++) {
            if(mf_classic_is_key_found(data, sector_num, MfClassicKeyB)) {
                furry_string_cat_printf(block_str, "%02X ", sec_tr->key_b[i]);
            } else {
                furry_string_cat_printf(block_str, "?? ");
            }
        }
        ...
        for(size_t i = 0; i < MF_CLASSIC_BLOCK_SIZE; i++) {
            if(mf_classic_is_block_read(data, block_num)) {
                furry_string_cat_printf(block_str, "%02X ", data->block[block_num].value[i]);
            } else {
                furry_string_cat_printf(block_str, "?? ");
            }
        }
    }
    furry_string_trim(block_str);
}
...
static bool nfc_device_save_mifare_classic_data(FlipperFormat* file, NfcDevice* dev) {
    ...
    do {
        ...
        FurryString* block_str;
        block_str = furry_string_alloc();
        for(size_t i = 0; i < blocks; i++) {
            furry_string_printf(temp_str, "Block %d", i);
            nfc_device_write_mifare_classic_block(block_str, data, i);
            if(!flipper_format_write_string(file, furry_string_get_cstr(temp_str), block_str)) {
                block_saved = false;
                break;
            }
        }
        furry_string_free(block_str);
...
static void nfc_device_load_mifare_classic_block(
    ...
    furry_string_trim(block_str);
    for(size_t i = 0; i < MF_CLASSIC_BLOCK_SIZE; i++) {
        char hi = furry_string_get_char(block_str, 3 * i);
        char low = furry_string_get_char(block_str, 3 * i + 1);
        uint8_t byte = 0;
        if(hex_char_to_uint8(hi, low, &byte)) {
            block_tmp.value[i] = byte;
        } else {
            FURRY_BIT_SET(block_unknown_bytes_mask, i);
        }
    }
...
static bool nfc_device_load_mifare_classic_data(FlipperFormat* file, NfcDevice* dev) {
    ...
    do {
        ...
        if(!furry_string_cmp(temp_str, "MINI")) {
            data->type = MfClassicTypeMini;
            data_blocks = 20;
        } else if(!furry_string_cmp(temp_str, "1K")) {
            data->type = MfClassicType1k;
            data_blocks = 64;
        } else if(!furry_string_cmp(temp_str, "4K")) {
        ...
        for(size_t i = 0; i < data_blocks; i++) {
            furry_string_printf(temp_str, "Block %d", i);
            if(!flipper_format_read_string(file, furry_string_get_cstr(temp_str), block_str)) {
                block_read = false;
                break;
            }
...
static void nfc_device_get_key_cache_file_path(NfcDevice* dev, FurryString* file_path) {
    uint8_t* uid = dev->dev_data.nfc_data.uid;
    uint8_t uid_len = dev->dev_data.nfc_data.uid_len;
    furry_string_set(file_path, NFC_DEVICE_KEYS_FOLDER "/");
    for(size_t i = 0; i < uid_len; i++) {
        furry_string_cat_printf(file_path, "%02X", uid[i]);
    }
    furry_string_cat_printf(file_path, NFC_DEVICE_KEYS_EXTENSION);
}
...
static bool nfc_device_save_mifare_classic_keys(NfcDevice* dev) {
    ...
    do {
        ...
        if(!storage_simply_remove(dev->storage, furry_string_get_cstr(temp_str))) break;
        if(!flipper_format_file_open_always(file, furry_string_get_cstr(temp_str))) break;
        ...
        for(size_t i = 0; (i < sector_num) && (key_save_success); i++) {
            ...
            if(FURRY_BIT(data->key_a_mask, i)) {
                furry_string_printf(temp_str, "Key A sector %d", i);
                key_save_success = flipper_format_write_hex(
                    file, furry_string_get_cstr(temp_str), sec_tr->key_a, 6);
            }
            if(!key_save_success) break;
            if(FURRY_BIT(data->key_b_mask, i)) {
                furry_string_printf(temp_str, "Key B sector %d", i);
                key_save_success = flipper_format_write_hex(
                    file, furry_string_get_cstr(temp_str), sec_tr->key_b, 6);
            }
        }
...
bool nfc_device_load_key_cache(NfcDevice* dev) {
    ...
    do {
        if(storage_common_stat(dev->storage, furry_string_get_cstr(temp_str), NULL) != FSE_OK)
            break;
        if(!flipper_format_file_open_existing(file, furry_string_get_cstr(temp_str))) break;
        uint32_t version = 0;
        if(!flipper_format_read_header(file, temp_str, &version)) break;
        if(furry_string_cmp_str(temp_str, nfc_keys_file_header)) break;
...
static void nfc_device_get_path_without_ext(FurryString* orig_path, FurryString* shadow_path) {
    // TODO: this won't work if there is ".nfc" anywhere in the path other than
    // at the end
    size_t ext_start = furry_string_search(orig_path, NFC_APP_EXTENSION);
    furry_string_set_n(shadow_path, orig_path, 0, ext_start);
}

static void nfc_device_get_shadow_path(FurryString* orig_path, FurryString* shadow_path) {
    nfc_device_get_path_without_ext(orig_path, shadow_path);
    furry_string_cat_printf(shadow_path, "%s", NFC_APP_SHADOW_EXTENSION);
}

static void nfc_device_get_folder_from_path(FurryString* path, FurryString* folder) {
    size_t last_slash = furry_string_search_rchar(path, '/');
    if(last_slash == FURRY_STRING_FAILURE) {
        // No slashes in the path, treat the whole path as a folder
        furry_string_set(folder, path);
    } else {
        furry_string_set_n(folder, path, 0, last_slash);
    }
}
...
bool nfc_device_save(NfcDevice* dev, const char* dev_name) {
    ...
    do {
        ...
        FurryString* folder = furry_string_alloc();
        // Get folder from filename (filename is in the form of "folder/filename.nfc", so the folder is "folder/")
        furry_string_set(temp_str, dev_name);
        // Get folder from filename
        nfc_device_get_folder_from_path(temp_str, folder);
        FURRY_LOG_I("Nfc", "Saving to folder %s", furry_string_get_cstr(folder));
        if(!storage_simply_mkdir(dev->storage, furry_string_get_cstr(folder))) {
            FURRY_LOG_E("Nfc", "Failed to create folder %s", furry_string_get_cstr(folder));
            break;
        }
        furry_string_free(folder);
...
bool nfc_device_save_shadow(NfcDevice* dev, const char* path) {
    ...
    FurryString* orig_path = furry_string_alloc();
    furry_string_set_str(orig_path, path);
    FurryString* shadow_path = furry_string_alloc();
    nfc_device_get_shadow_path(orig_path, shadow_path);

    bool file_saved = nfc_device_save(dev, furry_string_get_cstr(shadow_path));
    furry_string_free(orig_path);
    furry_string_free(shadow_path);

    return file_saved;
}

static bool nfc_device_load_data(NfcDevice* dev, FurryString* path, bool show_dialog) {
    bool parsed = false;
    FlipperFormat* file = flipper_format_file_alloc(dev->storage);
    FurryHalNfcDevData* data = &dev->dev_data.nfc_data;
    uint32_t data_cnt = 0;
    ...
    do {
        ...
        dev->shadow_file_exist =
            storage_common_stat(dev->storage, furry_string_get_cstr(temp_str), NULL) == FSE_OK;
        // Open shadow file if it exists. If not - open original
        if(dev->shadow_file_exist) {
            if(!flipper_format_file_open_existing(file, furry_string_get_cstr(temp_str))) break;
        } else {
            if(!flipper_format_file_open_existing(file, furry_string_get_cstr(path))) break;
        }
        // Read and verify file header
        uint32_t version = 0;
        if(!flipper_format_read_header(file, temp_str, &version)) break;
        if(furry_string_cmp_str(temp_str, nfc_file_header)) break;
...
bool nfc_device_load(NfcDevice* dev, const char* file_path, bool show_dialog) {
    ...
    // Load device data
    furry_string_set(dev->load_path, file_path);
    ...
    if(dev_load) {
        ...
        nfc_device_set_name(dev, furry_string_get_cstr(filename));
...
bool nfc_file_select(NfcDevice* dev) {
    furry_assert(dev);
    const char* folder = furry_string_get_cstr(dev->folder);
    ...
    if(res) {
        ...
        strncpy(dev->dev_name, furry_string_get_cstr(filename), NFC_DEV_NAME_MAX_LEN);
...
void nfc_device_data_clear(NfcDeviceData* dev_data) {
    ...
    furry_string_reset(dev_data->parsed_data);
}
...
void nfc_device_clear(NfcDevice* dev) {
    ...
    dev->format = NfcDeviceSaveFormatUid;
    furry_string_reset(dev->load_path);
}
...
bool nfc_device_delete(NfcDevice* dev, bool use_load_path) {
    ...
    do {
        ...
        if(use_load_path && !furry_string_empty(dev->load_path)) {
            furry_string_set(file_path, dev->load_path);
        } else {
            furry_string_printf(
                file_path,
                "%s/%s%s",
                furry_string_get_cstr(dev->folder),
                dev->dev_name,
                NFC_APP_EXTENSION);
        }
        if(!storage_simply_remove(dev->storage, furry_string_get_cstr(file_path))) break;
...
bool nfc_device_restore(NfcDevice* dev, bool use_load_path) {
    furry_assert(dev);
    furry_assert(dev->shadow_file_exist);
    ...
    do {
        ...
        if(!storage_simply_remove(dev->storage, furry_string_get_cstr(path))) break;
        dev->shadow_file_exist = false;
        if(use_load_path && !furry_string_empty(dev->load_path)) {
            furry_string_set(path, dev->load_path);
        } else {
            furry_string_printf(
                path,
                "%s/%s%s",
