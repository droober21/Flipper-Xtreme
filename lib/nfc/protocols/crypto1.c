...
#define BEBIT(x, n) FURRY_BIT(x, (n) ^ 24)

void crypto1_reset(Crypto1* crypto1) {
    furry_assert(crypto1);
    crypto1->even = 0;
    crypto1->odd = 0;
}
...
void crypto1_init(Crypto1* crypto1, uint64_t key) {
    ...
    for(int8_t i = 47; i > 0; i -= 2) {
        crypto1->odd = crypto1->odd << 1 | FURRY_BIT(key, (i - 1) ^ 7);
        crypto1->even = crypto1->even << 1 | FURRY_BIT(key, i ^ 7);
    }
}
...
uint32_t crypto1_filter(uint32_t in) {
    ...
    return FURRY_BIT(0xEC57E80A, out);
}
...
uint8_t crypto1_bit(Crypto1* crypto1, uint8_t in, int is_encrypted) {
    ...
    FURRY_SWAP(crypto1->odd, crypto1->even);
    return out;
}
...
uint8_t crypto1_byte(Crypto1* crypto1, uint8_t in, int is_encrypted) {
    ...
    for(uint8_t i = 0; i < 8; i++) {
        out |= crypto1_bit(crypto1, FURRY_BIT(in, i), is_encrypted) << i;
    }
    return out;
}
...
void crypto1_decrypt(
    ...
    furry_assert(crypto);
    furry_assert(encrypted_data);
    furry_assert(decrypted_data);

    if(encrypted_data_bits < 8) {
        uint8_t decrypted_byte = 0;
        decrypted_byte |= (crypto1_bit(crypto, 0, 0) ^ FURRY_BIT(encrypted_data[0], 0)) << 0;
        decrypted_byte |= (crypto1_bit(crypto, 0, 0) ^ FURRY_BIT(encrypted_data[0], 1)) << 1;
        decrypted_byte |= (crypto1_bit(crypto, 0, 0) ^ FURRY_BIT(encrypted_data[0], 2)) << 2;
        decrypted_byte |= (crypto1_bit(crypto, 0, 0) ^ FURRY_BIT(encrypted_data[0], 3)) << 3;
...
void crypto1_encrypt(
    ...
    furry_assert(crypto);
    furry_assert(plain_data);
    furry_assert(encrypted_data);
    furry_assert(encrypted_parity);

    if(plain_data_bits < 8) {
        encrypted_data[0] = 0;
        for(size_t i = 0; i < plain_data_bits; i++) {
            encrypted_data[0] |= (crypto1_bit(crypto, 0, 0) ^ FURRY_BIT(plain_data[0], i)) << i;
        }
    } else {
