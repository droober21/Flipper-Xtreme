...
static void emv_trace(FurryHalNfcTxRxContext* tx_rx, const char* message) {
    if(furry_log_get_level() == FurryLogLevelTrace) {
        FURRY_LOG_T(TAG, "%s", message);
        printf("TX: ");
...
static bool emv_decode_response(uint8_t* buff, uint16_t len, EmvApplication* app) {
    ...
    while(i < len) {
        ...
        if((first_byte & 31) == 31) { // 2-byte tag
            ...
            i++;
            FURRY_LOG_T(TAG, " 2-byte TLV EMV tag: %x", tag);
        } else {
            tag = buff[i];
            FURRY_LOG_T(TAG, " 1-byte TLV EMV tag: %x", tag);
        }
        i++;
        tlen = buff[i];
        if((tlen & 128) == 128) { // long length value
            i++;
            tlen = buff[i];
            FURRY_LOG_T(TAG, " 2-byte TLV length: %d", tlen);
        } else {
            FURRY_LOG_T(TAG, " 1-byte TLV length: %d", tlen);
        }
        i++;
        if((first_byte & 32) == 32) { // "Constructed" -- contains more TLV data to parse
            FURRY_LOG_T(TAG, "Constructed TLV %x", tag);
            if(!emv_decode_response(&buff[i], tlen, app)) {
                FURRY_LOG_T(TAG, "Failed to decode response for %x", tag);
                // return false;
            } else {
                success = true;
            }
            ...
            case EMV_TAG_AID:
                ...
                success = true;
                FURRY_LOG_T(TAG, "found EMV_TAG_AID %x", tag);
                break;
            ...
            case EMV_TAG_CARD_NAME:
                ...
                app->name_found = true;
                success = true;
                FURRY_LOG_T(TAG, "found EMV_TAG_CARD_NAME %x : %s", tag, app->name);
                break;
            case EMV_TAG_PDOL:
                memcpy(app->pdol.data, &buff[i], tlen);
                app->pdol.size = tlen;
                success = true;
                FURRY_LOG_T(TAG, "found EMV_TAG_PDOL %x (len=%d)", tag, tlen);
                break;
            case EMV_TAG_AFL:
                memcpy(app->afl.data, &buff[i], tlen);
                app->afl.size = tlen;
                success = true;
                FURRY_LOG_T(TAG, "found EMV_TAG_AFL %x (len=%d)", tag, tlen);
                break;
            case EMV_TAG_TRACK_1_EQUIV: {
                ...
                success = true;
                FURRY_LOG_T(TAG, "found EMV_TAG_TRACK_1_EQUIV %x : %s", tag, track_1_equiv);
                break;
            }
            case EMV_TAG_TRACK_2_EQUIV: {
                ...
                success = true;
                FURRY_LOG_T(TAG, "found EMV_TAG_TRACK_2_EQUIV %x : %s", tag, track_2_equiv);
                break;
            }
            case EMV_TAG_PAN:
...
static bool emv_select_ppse(FurryHalNfcTxRxContext* tx_rx, EmvApplication* app) {
    ...
    FURRY_LOG_D(TAG, "Send select PPSE");
    if(furry_hal_nfc_tx_rx(tx_rx, 300)) {
        ...
        if(emv_decode_response(tx_rx->rx_data, tx_rx->rx_bits / 8, app)) {
            ...
            FURRY_LOG_E(TAG, "Failed to parse application");
        }
    } else {
        FURRY_LOG_E(TAG, "Failed select PPSE");
    }

    return app_aid_found;
}

static bool emv_select_app(FurryHalNfcTxRxContext* tx_rx, EmvApplication* app) {
    ...
    FURRY_LOG_D(TAG, "Start application");
    if(furry_hal_nfc_tx_rx(tx_rx, 300)) {
        emv_trace(tx_rx, "Start application answer:");
        if(emv_decode_response(tx_rx->rx_data, tx_rx->rx_bits / 8, app)) {
            app->app_started = true;
        } else {
            FURRY_LOG_E(TAG, "Failed to read PAN or PDOL");
        }
    } else {
        FURRY_LOG_E(TAG, "Failed to start application");
    }
...
static bool emv_get_processing_options(FurryHalNfcTxRxContext* tx_rx, EmvApplication* app) {
    ...
    FURRY_LOG_D(TAG, "Get proccessing options");
    if(furry_hal_nfc_tx_rx(tx_rx, 300)) {
        ...
        FURRY_LOG_E(TAG, "Failed to get processing options");
    }
...
static bool emv_read_sfi_record(
    ...
    if(furry_hal_nfc_tx_rx(tx_rx, 300)) {
        ...
        FURRY_LOG_E(TAG, "Failed to read SFI record %d", record_num);
    }

    return card_num_read;
}

static bool emv_read_files(FurryHalNfcTxRxContext* tx_rx, EmvApplication* app) {
    bool card_num_read = false;
    ...
    FURRY_LOG_D(TAG, "Search PAN in SFI");
...
bool emv_card_emulation(FurryHalNfcTxRxContext* tx_rx) {
    ...
    do {
        FURRY_LOG_D(TAG, "Read select PPSE command");
