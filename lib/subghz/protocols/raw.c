...
bool subghz_protocol_raw_save_to_file_init(
    ...
    do {
        ...
        furry_string_set(instance->file_name, dev_name);
        // First remove subghz device file if it was saved
        furry_string_printf(temp_str, "%s/%s%s", SUBGHZ_RAW_FOLDER, dev_name, SUBGHZ_APP_EXTENSION);

        if(!storage_simply_remove(instance->storage, furry_string_get_cstr(temp_str))) {
            break;
        }

        // Open file
        if(!flipper_format_file_open_always(
               instance->flipper_file, furry_string_get_cstr(temp_str))) {
            FURRY_LOG_E(TAG, "Unable to open file for write: %s", furry_string_get_cstr(temp_str));
            break;
        }
        ...
        if(!flipper_format_write_string_cstr(
               instance->flipper_file, "Preset", furry_string_get_cstr(temp_str))) {
...
static bool subghz_protocol_raw_save_to_file_write(SubGhzProtocolDecoderRAW* instance) {
    ...
    if(instance->file_is_open == RAWFileIsOpenWrite) {
        if(!flipper_format_write_int32(
            ...
            FURRY_LOG_E(TAG, "Unable to add RAW_Data");
        } else {
...
void* subghz_protocol_decoder_raw_alloc(SubGhzEnvironment* environment) {
    ...
    instance->last_level = false;
    instance->file_is_open = RAWFileIsOpenClose;
    instance->file_name = furry_string_alloc();
...
void subghz_protocol_decoder_raw_free(void* context) {
    ...
    furry_string_free(instance->file_name);
    free(instance);
}
...
void subghz_protocol_decoder_raw_get_string(void* context, FurryString* output) {
    ...
    UNUSED(context);
    //ToDo no use
    furry_string_cat_printf(output, "RAW Data");
}
...
static bool subghz_protocol_encoder_raw_worker_init(SubGhzProtocolEncoderRAW* instance) {
    ...
    if(subghz_file_encoder_worker_start(
           instance->file_worker_encoder, furry_string_get_cstr(instance->file_name))) {
...
void subghz_protocol_raw_gen_fff_data(FlipperFormat* flipper_format, const char* file_path) {
    do {
        ...
        if(!flipper_format_write_string_cstr(flipper_format, "File_name", file_path)) {
            FURRY_LOG_E(TAG, "Unable to add File_name");
            break;
        }
    } while(false);
}
...
SubGhzProtocolStatus
    ...
    do {
        ...
        furry_string_set(instance->file_name, temp_str);
