...
void subghz_keystore_free(SubGhzKeystore* instance) {
    ...
    for
        M_EACH(manufacture_code, instance->data, SubGhzKeyArray_t) {
            furry_string_free(manufacture_code->name);
...
static void subghz_keystore_add_key(
    ...
    manufacture_code->name = furry_string_alloc_set(name);
...
static bool subghz_keystore_process_line(SubGhzKeystore* instance, char* line) {
    ...
    if(ret == 3) {
        ...
        FURRY_LOG_E(TAG, "Failed to load line: %s\r\n", line);
        return false;
    }
}
...
static bool subghz_keystore_read_file(SubGhzKeystore* instance, Stream* stream, uint8_t* iv) {
    ...
    do {
        if(iv) {
            if(!furry_hal_crypto_store_load_key(SUBGHZ_KEYSTORE_FILE_ENCRYPTION_KEY_SLOT, iv)) {
                FURRY_LOG_E(TAG, "Unable to load decryption key");
                break;
            }
        }
                ...
                if(buffer[i] == '\n' && encrypted_line_cursor > 0) {
                    ...
                    if(iv) {
                        ...
                        if(len % 32 == 0) {
                            ...
                            FURRY_LOG_E(TAG, "Invalid encrypted data: %s", encrypted_line);
                        }
                    } else {
                    ...
                    if(encrypted_line_cursor < SUBGHZ_KEYSTORE_FILE_ENCRYPTED_LINE_SIZE) {
                        ...
                        FURRY_LOG_E(TAG, "Malformed file");
                        result = false;
                        break;
                    }
                }
        ...
        if(iv) furry_hal_crypto_store_unload_key(SUBGHZ_KEYSTORE_FILE_ENCRYPTION_KEY_SLOT);
    } while(false);
...
bool subghz_keystore_load(SubGhzKeystore* instance, const char* file_name) {
    ...
    FURRY_LOG_I(TAG, "Loading keystore %s", file_name);
    ...
    do {
        if(!flipper_format_file_open_existing(flipper_format, file_name)) {
            FURRY_LOG_E(TAG, "Unable to open file for read: %s", file_name);
            break;
        }
        ...
        if(!flipper_format_read_uint32(flipper_format, "Encryption", (uint32_t*)&encryption, 1)) {
            FURRY_LOG_E(TAG, "Missing encryption type");
            break;
        }

        if(strcmp(furry_string_get_cstr(filetype), SUBGHZ_KEYSTORE_FILE_TYPE) != 0 ||
        ...
        if(encryption == SubGhzKeystoreEncryptionNone) {
            ...
            FURRY_LOG_E(TAG, "Unknown encryption");
            break;
        }
    } while(0);
...
bool subghz_keystore_save(SubGhzKeystore* instance, const char* file_name, uint8_t* iv) {
    ...
    do {
        if(!flipper_format_file_open_always(flipper_format, file_name)) {
            FURRY_LOG_E(TAG, "Unable to open file for write: %s", file_name);
            break;
        }
        ...
        if(!flipper_format_write_uint32(flipper_format, "Encryption", &encryption, 1)) {
            FURRY_LOG_E(TAG, "Unable to add Encryption");
            break;
        }
        if(!flipper_format_write_hex(flipper_format, "IV", iv, 16)) {
            FURRY_LOG_E(TAG, "Unable to add IV");
            break;
        }
        ...
        for
            M_EACH(key, instance->data, SubGhzKeyArray_t) {
                ...
                int len = snprintf(
                    ...
                    (uint32_t)key->key,
                    key->type,
                    furry_string_get_cstr(key->name));
                // Verify length and align
                furry_assert(len > 0);
                if(len % 16 != 0) {
                    len += (16 - len % 16);
                }
                furry_assert(len % 16 == 0);
                furry_assert(len <= SUBGHZ_KEYSTORE_FILE_DECRYPTED_LINE_SIZE);
                // Form encrypted line
                if(!furry_hal_crypto_encrypt(
        ...
        furry_hal_crypto_store_unload_key(SUBGHZ_KEYSTORE_FILE_ENCRYPTION_KEY_SLOT);
        size_t total_keys = SubGhzKeyArray_size(instance->data);
        result = encrypted_line_count == total_keys;
        if(result) {
            FURRY_LOG_I(TAG, "Success. Encrypted: %zu of %zu", encrypted_line_count, total_keys);
        } else {
            FURRY_LOG_E(TAG, "Failure. Encrypted: %zu of %zu", encrypted_line_count, total_keys);
        }
    } while(0);
    flipper_format_free(flipper_format);
...
bool subghz_keystore_raw_encrypted_save(
    ...
    do {
        if(!flipper_format_file_open_existing(input_flipper_format, input_file_name)) {
            FURRY_LOG_E(TAG, "Unable to open file for read: %s", input_file_name);
            break;
        }
        ...
        if(strcmp(furry_string_get_cstr(filetype), SUBGHZ_KEYSTORE_FILE_RAW_TYPE) != 0 ||
           version != SUBGHZ_KEYSTORE_FILE_VERSION) {
            FURRY_LOG_E(TAG, "Type or version mismatch");
            break;
        }
        ...
        if(encryption != SubGhzKeystoreEncryptionNone) {
            FURRY_LOG_E(TAG, "Already encryption");
            break;
        }
        ...
        if(!flipper_format_file_open_always(output_flipper_format, output_file_name)) {
            FURRY_LOG_E(TAG, "Unable to open file for write: %s", output_file_name);
            break;
        }
        if(!flipper_format_write_header_cstr(
               output_flipper_format,
               furry_string_get_cstr(filetype),
        ...
        if(!flipper_format_write_string_cstr(output_flipper_format, "Encrypt_data", "RAW")) {
            FURRY_LOG_E(TAG, "Unable to add Encrypt_data");
            break;
        }
        ...
        size_t ret = 0;
        furry_assert(FILE_BUFFER_SIZE % 16 == 0);
...
bool subghz_keystore_raw_get_data(const char* file_name, size_t offset, uint8_t* data, size_t len) {
    ...
    FurryString* str_temp;
    str_temp = furry_string_alloc();
    ...
    do {
        ...
        if(strcmp(furry_string_get_cstr(str_temp), SUBGHZ_KEYSTORE_FILE_RAW_TYPE) != 0 ||
           version != SUBGHZ_KEYSTORE_FILE_VERSION) {
            FURRY_LOG_E(TAG, "Type or version mismatch");
            break;
        }
        ...
        if(!flipper_format_read_string(flipper_format, "Encrypt_data", str_temp)) {
            FURRY_LOG_E(TAG, "Missing Encrypt_data");
            break;
        }
        ...
        furry_assert(SUBGHZ_KEYSTORE_FILE_DECRYPTED_LINE_SIZE >= bufer_size / 2);
        ...
        if(size < (offset * 2 + len * 2)) {
            FURRY_LOG_E(TAG, "Seek position exceeds file size");
            break;
        }
