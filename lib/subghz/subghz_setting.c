...
typedef struct {
    FurryString* custom_preset_name;
...
static void subghz_setting_preset_reset(SubGhzSetting* instance) {
    for
        M_EACH(item, instance->preset->data, SubGhzSettingCustomPresetItemArray_t) {
            furry_string_free(item->custom_preset_name);
...
static void subghz_setting_load_default_preset(
    ...
    item->custom_preset_name = furry_string_alloc();
    furry_string_set(item->custom_preset_name, preset_name);
...
static void subghz_setting_load_default_region(
    ...
    subghz_setting_load_default_preset(
        instance,
        "AM270",
        (uint8_t*)furry_hal_subghz_preset_ook_270khz_async_regs,
        furry_hal_subghz_preset_ook_async_patable);
    subghz_setting_load_default_preset(
        instance,
        "AM650",
        (uint8_t*)furry_hal_subghz_preset_ook_650khz_async_regs,
        furry_hal_subghz_preset_ook_async_patable);
    subghz_setting_load_default_preset(
        instance,
        "FM238",
        (uint8_t*)furry_hal_subghz_preset_2fsk_dev2_38khz_async_regs,
        furry_hal_subghz_preset_2fsk_async_patable);
    subghz_setting_load_default_preset(
        instance,
        "FM476",
...
void subghz_setting_load(SubGhzSetting* instance, const char* file_path) {
    ...
    if(file_path) {
        do {
            if(!flipper_format_file_open_existing(fff_data_file, file_path)) {
                FURRY_LOG_I(TAG, "File is not used %s", file_path);
                break;
            }
            ...
            if((!strcmp(furry_string_get_cstr(temp_str), SUBGHZ_SETTING_FILE_TYPE)) &&
            ...
            if(!temp_bool) {
                FURRY_LOG_I(TAG, "Removing standard frequencies");
                FrequencyList_reset(instance->frequencies);
                FrequencyList_reset(instance->hopper_frequencies);
            } else {
                FURRY_LOG_I(TAG, "Keeping standard frequencies");
            }
            ...
            while(flipper_format_read_uint32(
                ...
                if(furry_hal_subghz_is_frequency_valid(temp_data32)) {
                    FURRY_LOG_I(TAG, "Frequency loaded %lu", temp_data32);
                    FrequencyList_push_back(instance->frequencies, temp_data32);
                } else {
                    FURRY_LOG_E(TAG, "Frequency not supported %lu", temp_data32);
                }
            }
            ...
            while(flipper_format_read_uint32(
                ...
                if(furry_hal_subghz_is_frequency_valid(temp_data32)) {
                    FURRY_LOG_I(TAG, "Hopper frequency loaded %lu", temp_data32);
                    FrequencyList_push_back(instance->hopper_frequencies, temp_data32);
                } else {
                    FURRY_LOG_E(TAG, "Hopper frequency not supported %lu", temp_data32);
                }
            }
            ...
            while(flipper_format_read_string(fff_data_file, "Custom_preset_name", temp_str)) {
                FURRY_LOG_I(TAG, "Custom preset loaded %s", furry_string_get_cstr(temp_str));
                subghz_setting_load_custom_preset(
                    instance, furry_string_get_cstr(temp_str), fff_data_file);
            }
    ...
    if(!FrequencyList_size(instance->frequencies) ||
       !FrequencyList_size(instance->hopper_frequencies)) {
        FURRY_LOG_E(TAG, "Error loading user settings, loading default settings");
        subghz_setting_load_default(instance);
    }
}
...
const char* subghz_setting_get_preset_name(SubGhzSetting* instance, size_t idx) {
    ...
    return furry_string_get_cstr(item->custom_preset_name);
}
...
int subghz_setting_get_inx_preset_by_name(SubGhzSetting* instance, const char* preset_name) {
    ...
    for
        M_EACH(item, instance->preset->data, SubGhzSettingCustomPresetItemArray_t) {
            if(strcmp(furry_string_get_cstr(item->custom_preset_name), preset_name) == 0) {
                return idx;
            }
            idx++;
        }
    furry_crash("SubGhz: No name preset.");
    return -1;
}
...
bool subghz_setting_load_custom_preset(
    ...
    do {
        ...
        if(!temp_data32 || (temp_data32 % 2)) {
            FURRY_LOG_E(TAG, "Integrity error Custom_preset_data");
            break;
        }
        ...
        if(!flipper_format_read_hex(
            ...
            FURRY_LOG_E(TAG, "Missing Custom_preset_data");
            break;
        }
        return true;
    } while(true);
