...
void pretty_format_bytes_hex_canonical(
    ...
    furry_string_reset(result);
    furry_string_reserve(result, line_count * line_length);

    for(size_t i = 0; i < data_size; i += num_places) {
        if(line_prefix) {
            furry_string_cat(result, line_prefix);
        }
        ...
        for(size_t j = begin_idx; j < end_idx; j++) {
            furry_string_cat_printf(result, "%02X ", data[j]);
        }

        furry_string_push_back(result, '|');

        for(size_t j = begin_idx; j < end_idx; j++) {
            const char c = data[j];
            const char sep = ((j < end_idx - 1) ? ' ' : '\n');
            const char* fmt = ((j < data_size - 1) ? "%c%c" : "%c");
            furry_string_cat_printf(result, fmt, (c > 0x1f && c < 0x7f) ? c : '.', sep);
        }
    }
